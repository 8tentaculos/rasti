(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const Ke=!1;function ne(e){if(typeof e!="function"){const t="Event listener must be a function";throw new TypeError(t)}}class ze{on(t,s){return ne(s),this.listeners||(this.listeners={}),this.listeners[t]||(this.listeners[t]=[]),this.listeners[t].push(s),()=>this.off(t,s)}once(t,s){ne(s);const n=(...a)=>{s(...a),this.off(t,n)};return this.on(t,n)}off(t,s){if(this.listeners){if(!t){delete this.listeners;return}this.listeners[t]&&(s?(this.listeners[t]=this.listeners[t].filter(n=>n!==s),this.listeners[t].length||delete this.listeners[t]):delete this.listeners[t],Object.keys(this.listeners).length||delete this.listeners)}}emit(t,...s){!this.listeners||!this.listeners[t]||this.listeners[t].slice().forEach(n=>n(...s))}listenTo(t,s,n){return t.on(s,n),this.listeningTo||(this.listeningTo=[]),this.listeningTo.push({emitter:t,type:s,listener:n}),()=>this.stopListening(t,s,n)}listenToOnce(t,s,n){ne(n);const a=(...o)=>{n(...o),this.stopListening(t,s,a)};return this.listenTo(t,s,a)}stopListening(t,s,n){this.listeningTo&&(this.listeningTo=this.listeningTo.filter(a=>!t||t===a.emitter&&!s||t===a.emitter&&s===a.type&&!n||t===a.emitter&&s===a.type&&n===a.listener?(a.emitter.off(a.type,a.listener),!1):!0),this.listeningTo.length||delete this.listeningTo)}}const J=(e,t,...s)=>typeof e!="function"?e:e.apply(t,s);class ie extends ze{constructor(){super(),this.preinitialize.apply(this,arguments),this.attributes=Object.assign({},J(this.defaults,this),this.parse.apply(this,arguments)),this.previous={},Object.keys(this.attributes).forEach(this.defineAttribute.bind(this))}preinitialize(){}defineAttribute(t){Object.defineProperty(this,`${this.constructor.attributePrefix}${t}`,{get:()=>this.get(t),set:s=>{this.set(t,s)}})}get(t){return this.attributes[t]}set(t,s,...n){let a,o;typeof t=="object"?(a=t,o=[s,...n]):(a={[t]:s},o=n);const l=this._changing;this._changing=!0;const r={};l||(this.previous=Object.assign({},this.attributes)),Object.keys(a).forEach(p=>{a[p]!==this.attributes[p]&&(r[p]=a[p],this.attributes[p]=a[p])});const c=Object.keys(r);if(c.length&&(this._pending=["change",this,r,...o]),c.forEach(p=>{this.emit(`change:${p}`,this,a[p],...o)}),l)return this;for(;this._pending;){const p=this._pending;this._pending=null,this.emit.apply(this,p)}return this._pending=null,this._changing=!1,this}parse(t){return t}toJSON(){return Object.assign({},this.attributes)}}ie.attributePrefix="";const Ye=["el","tag","attributes","events","model","template","onDestroy"];class oe extends ze{constructor(t={}){super(),this.preinitialize.apply(this,arguments),this.delegatedEventListeners=[],this.children=[],this.destroyQueue=[],this.viewOptions=[],Ye.forEach(s=>{s in t&&(this[s]=t[s],this.viewOptions.push(s))}),this.ensureUid(),this.ensureElement()}preinitialize(){}$(t){return this.el.querySelector(t)}$$(t){return this.el.querySelectorAll(t)}destroy(){return this.destroyChildren(),this.undelegateEvents(),this.stopListening(),this.off(),this.destroyQueue.forEach(t=>t()),this.destroyQueue=[],this.onDestroy.apply(this,arguments),this.destroyed=!0,this}onDestroy(){}addChild(t){return this.children.push(t),t}destroyChildren(){this.children.forEach(t=>t.destroy()),this.children=[]}ensureUid(){this.uid||(this.uid=`r${++oe.uid}`)}ensureElement(){if(this.el)this.el=J(this.el,this);else{const t=J(this.tag,this),s=J(this.attributes,this);this.el=this.createElement(t,s)}this.delegateEvents()}createElement(t="div",s={}){let n=document.createElement(t);return Object.keys(s).forEach(a=>n.setAttribute(a,s[a])),n}removeElement(){return this.el.parentNode.removeChild(this.el),this}delegateEvents(t){if(t||(t=J(this.events,this)),!t)return this;this.delegatedEventListeners.length&&this.undelegateEvents();const s={};return Object.keys(t).forEach(n=>{const a=n.match(/^(\w+)(?:\s+(.+))*$/);if(!a){const c=`Invalid event format: ${n}`;throw new Error(c)}const[,o,l]=a;let r=t[n];typeof r=="string"&&(r=this[r]),ne(r),s[o]||(s[o]=[]),s[o].push([l,r])}),Object.keys(s).forEach(n=>{const a=o=>{let l=o.target;for(;l;)l.matches&&s[n].forEach(([r,c])=>{(l===this.el&&!r||l!==this.el&&l.matches(r))&&c.call(this,o,this,l)}),l=l===this.el||o.cancelBubble?null:l.parentElement};this.delegatedEventListeners.push([n,a]),this.el.addEventListener(n,a)}),this}undelegateEvents(){return this.delegatedEventListeners.forEach(([t,s])=>{this.el.removeEventListener(t,s)}),this.delegatedEventListeners=[],this}render(){return this}static sanitize(t){return`${t}`.replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"})[s])}static resetUid(){oe.uid=0}}oe.uid=0;class we{constructor(t){this.value=t}toString(){return this.value}}class ge{constructor(t){this.items=t}}class Je{constructor(){this.listeners=[],this.types=new Set,this.previousSize=0}addListener(t,s){return this.types.add(s),this.listeners.push(t),this.listeners.length-1}reset(){this.listeners=[],this.previousSize=this.types.size}hasPendingTypes(){return this.types.size>this.previousSize}}function Xe(e,t={}){const s={},n=[];return Object.keys(e).forEach(a=>{let o=e[a];o!==t[a]&&(o===!0?s[a]="":o!==!1&&((o===null||typeof o>"u")&&(o=""),s[a]=o))}),Object.keys(t).forEach(a=>{(!(a in e)||t[a]!==e[a]&&e[a]===!1)&&n.push(a)}),{add:s,remove:n}}const xe=["value","checked","selected"];let Ge=class{constructor(t){this.getSelector=t.getSelector,this.getAttributes=t.getAttributes,this.previousAttributes={}}hydrate(t){this.ref=t.querySelector(this.getSelector())}update(){const t=this.getAttributes(),{remove:s,add:n}=Xe(t,this.previousAttributes);this.previousAttributes=t,s.forEach(a=>{this.ref.removeAttribute(a),xe.indexOf(a)!==-1&&a in this.ref&&(this.ref[a]=a==="value"?"":!1)}),Object.keys(n).forEach(a=>{const o=n[a];this.ref.setAttribute(a,o),xe.indexOf(a)!==-1&&a in this.ref&&(this.ref[a]=a==="value"?o:o!==!1&&o!=="false")})}};class Ze{constructor(){}reset(){this.paused=0,this.previous=this.tracked||new Map,this.tracked=new Map,this.positionStack=[0]}push(){this.positionStack.push(0)}pop(){this.positionStack.pop()}increment(){this.positionStack[this.positionStack.length-1]++}pause(){this.paused++}resume(){this.paused--}getPath(){return this.positionStack.join("-")}track(t){return this.paused===0&&this.tracked.set(this.getPath(),t),t}hasSingleComponent(){if(this.tracked.size!==1||this.previous.size!==1)return!1;const[t,s]=this.tracked.entries().next().value,[n,a]=this.previous.entries().next().value;return t!=="0"||n!=="0"?!1:s===a}findRecyclable(t){const s=this.previous.get(this.getPath());return s&&!s.key&&s.constructor===t.constructor?s:null}}const $e=["value","checked","selected"],Qe=(e,t)=>{const s=e.childNodes,n=t.childNodes,a=s.length;if(a!==n.length)return!1;for(let o=0;o<a;o++)if(!s[o].isEqualNode(n[o]))return!1;return!0},et=(e,t)=>{const s=t.attributes,n=e.attributes,a=new Set;for(let o=0,l=s.length;o<l;o++){const{name:r,value:c}=s[o];a.add(r),e.getAttribute(r)!==c&&e.setAttribute(r,c)}for(let o=n.length-1;o>=0;o--){const{name:l}=n[o];a.has(l)||e.removeAttribute(l)}for(let o=0,l=$e.length;o<l;o++){const r=$e[o];r in e&&e[r]!==t[r]&&(e[r]=t[r])}},tt=(e,t)=>{const s=Array.from(t.childNodes);e.replaceChildren(...s)};function st(e,t){if(e.nodeType!==t.nodeType){e.replaceWith(t);return}if(e.nodeType===Node.TEXT_NODE){e.nodeValue!==t.nodeValue&&(e.nodeValue=t.nodeValue);return}if(e.tagName!==t.tagName){e.replaceWith(t);return}et(e,t),Qe(e,t)||tt(e,t)}function fe(e,t,s=()=>!1,n){let a=n||e.firstChild;for(;a;){if(a.nodeType===Node.COMMENT_NODE&&a.data.trim()===t)return a;if(a.nodeType===Node.ELEMENT_NODE&&!s(a)&&a.firstChild){a=a.firstChild;continue}for(;a&&!a.nextSibling;)if(a=a.parentNode,!a||a===e)return null;a&&(a=a.nextSibling)}return null}class ke{constructor(t){this.getStart=t.getStart,this.getEnd=t.getEnd,this.expression=t.expression,this.shouldSkipFind=t.shouldSkipFind,this.shouldSkipSync=t.shouldSkipSync,this.tracker=new Ze}hydrate(t){const s=fe(t,this.getStart(),this.shouldSkipFind),n=fe(t,this.getEnd(),this.shouldSkipFind,s);this.ref=[s,n]}update(t,s){let n;const[a,o]=this.ref,l=a.nextSibling,r=l===o,c=!r&&l.nextSibling===o;if(r?o.parentNode.insertBefore(t,o):c&&t.children.length===1&&!this.shouldSkipSync(l)&&!this.shouldSkipSync(t.firstChild)?st(l,t.firstChild):(n=document.createComment(""),o.parentNode.insertBefore(n,o),o.parentNode.insertBefore(t,o)),s(),n)if(this.ref[0].nextSibling===n)n.parentNode.removeChild(n);else{const x=document.createRange();x.setStartAfter(this.ref[0]),x.setEndAfter(n),x.deleteContents()}}updateElement(t,s,n){const a=document.createComment("");t.parentNode.insertBefore(a,t.nextSibling),a.parentNode.insertBefore(s.firstChild,a.nextSibling),n(),t.nextSibling===a&&t.parentNode.removeChild(t),a.parentNode.removeChild(a)}}const Be=e=>e.reduce((t,s)=>(Array.isArray(s)?t.push(...Be(s)):t.push(s),t),[]);function ce(e){const t=document.createElement("template");return t.innerHTML=`${e}`.trim(),t.content}function Pe(e){const t=[];return Object.keys(e).forEach(s=>{let n=e[s];n===!0?t.push(s):n!==!1&&((n===null||typeof n>"u")&&(n=""),t.push(`${s}="${n}"`))}),t.join(" ")}let de,re,Ne,Ue;typeof document<"u"&&(de=!!navigator.userAgent.match(/Chrome/),re=!!Element.prototype.moveBefore,Ne=!re||de,Ue=re&&de);function at(e,t){const s=Ne&&document.activeElement&&t.contains(document.activeElement)?document.activeElement:null;s&&Ue&&s.blur(),e.parentNode[re?"moveBefore":"insertBefore"](t,e),e.parentNode.removeChild(e),s&&s!==document.activeElement&&t.contains(s)&&s.focus()}const G=(e,t,s)=>{try{return typeof e!="function"?e:(Ke&&e.prototype instanceof f,e.call(t,t))}catch(n){if(s&&!n._rasti){let a;a=`Error in ${t.constructor.name}#${t.uid} expression`;const o=new Error(a,{cause:n});throw o._rasti=!0,o}throw n}},Ce=e=>!!(e&&e.dataset&&e.dataset[f.DATASET_ELEMENT]&&e.dataset[f.DATASET_ELEMENT].endsWith("-1")),nt=e=>!!(e&&(e.dataset&&e.dataset[f.DATASET_ELEMENT]||e.querySelector&&e.querySelector(`[${f.ATTRIBUTE_ELEMENT}]`))),Te=(e,t)=>e.reduce((s,n,a)=>(s.push(n),typeof t[a]<"u"&&s.push(f.PLACEHOLDER(a)),s),[]).join(""),ve=(e,t)=>{const s=f.PLACEHOLDER("(\\d+)"),n=e.match(new RegExp(`^${s}$`));if(n)return[t[parseInt(n[1],10)]];const a=new RegExp(`${s}`,"g"),o=[];let l=0,r;for(;(r=a.exec(e))!==null;){const c=e.slice(l,r.index);o.push(f.markAsSafeHTML(c),t[parseInt(r[1],10)]),l=r.index+r[0].length}return o.push(f.markAsSafeHTML(e.slice(l))),o},je=(e,t)=>e.reduce((s,n)=>{const a=t(n[0]);if(n.length===1)typeof a=="object"?s=Object.assign(s,a):typeof a=="string"&&(s[a]=!0);else{const o=n[2]?t(n[1]):n[1];s[a]=o}return s},{}),Fe=(e,t,s)=>{const n={};return Object.keys(e).forEach(a=>{const o=a.match(/on(([A-Z]{1}[a-z]+)+)/);if(o&&o[1]){const l=o[1].toLowerCase(),r=e[a];if(r){const c=t.addListener(r,l);n[s(l)]=c}}else n[a]=e[a]}),n},be=(e,t,s=!1)=>{const n=f.PLACEHOLDER("(\\d+)"),a=new Map;return s||(e=e.replace(new RegExp(n,"g"),(o,l)=>{const r=t[l];if(r&&r.prototype instanceof f){if(a.has(r))return a.get(r);a.set(r,o)}return o})),e.replace(new RegExp(`<(${n})([^>]*)/>|<(${n})([^>]*)>([\\s\\S]*?)</\\4>`,"g"),(o,l,r,c,p,x,u,d)=>{let h,b,E;if(p?(h=t[x],b=u):(h=typeof r<"u"?t[r]:l,b=c),!(h.prototype instanceof f))return o;if(p){const V=be(d,t,!0),z=qe(V,t);E=ve(z,t)}const N=_e(b,t),R=function(){const V=je(N,z=>G(z,this,"children options"));return E&&(V.renderChildren=()=>new ge(E.map(z=>G(z,this,"children")))),h.mount(V)};return t.push(R),f.PLACEHOLDER(t.length-1)})},We=(e,t)=>{const s=f.PLACEHOLDER("(?:\\d+)");return e.replace(new RegExp(`<(${s}|[a-z]+[1-6]?)(?:\\s*)((?:"[^"]*"|'[^']*'|[^>])*)(/?>)`,"gi"),t)},ot=(e,t,s)=>{const n=f.PLACEHOLDER("(?:\\d+)");if(e.match(new RegExp(`^\\s*${n}\\s*$`)))return e;const o=e.match(new RegExp(`^\\s*<([a-z]+[1-6]?|${n})([^>]*)>([\\s\\S]*?)</(\\1|${n})>\\s*$|^\\s*<([a-z]+[1-6]?|${n})([^>]*)/>\\s*$`));if(!o){const r="Invalid component template";throw new Error(r)}let l=0;return We(o[0],(r,c,p,x)=>{const u=l===0,d=++l;if(!u&&!p.match(new RegExp(n)))return r;const h=_e(p,t),b=z=>`${z}-${d}`,E=function(){const z=Fe(je(h,K=>G(K,this,"element attribute")),this.eventsManager,K=>f.ATTRIBUTE_EVENT(K,this.uid));return u&&this.attributes&&Object.assign(z,J(this.attributes,this)),z[f.ATTRIBUTE_ELEMENT]=b(this.uid),z},N=function(){return`[${f.ATTRIBUTE_ELEMENT}="${b(this.uid)}"]`},R=s.length;s.push({getSelector:N,getAttributes:E}),t.push(function(){const z=this.template.elements[R],K=z.getAttributes.call(this);return z.previousAttributes=K,f.markAsSafeHTML(Pe(K))});const V=f.PLACEHOLDER(t.length-1);return`<${c} ${V}${x}`})},qe=(e,t)=>{const s=f.PLACEHOLDER("(?:\\d+)");return We(e,(n,a,o,l)=>{if(!o.match(new RegExp(s)))return n;const r=_e(o,t),c=function(){return Fe(je(r,u=>G(u,this,"partial element attribute")),this.eventsManager,u=>f.ATTRIBUTE_EVENT(u,this.uid))};t.push(function(){const x=c.call(this);return f.markAsSafeHTML(Pe(x))});const p=f.PLACEHOLDER(t.length-1);return`<${a} ${p}${l}`})},lt=(e,t,s)=>{const n=f.PLACEHOLDER("(\\d+)");let a=0;return e.replace(new RegExp(n,"g"),function(o,l,r){const c=e.substring(0,r),p=c.lastIndexOf("<"),x=c.lastIndexOf(">");if(p>x)return o;const u=++a;function d(){return f.MARKER_START(`${this.uid}-${u}`)}function h(){return f.MARKER_END(`${this.uid}-${u}`)}const b=s.length;return s.push({getStart:d,getEnd:h,expression:t[l]}),t.push(function(){return this.template.interpolations[b]}),f.PLACEHOLDER(t.length-1)})},_e=(e,t)=>{const s=f.PLACEHOLDER("(\\d+)"),n=[],a=new RegExp(`(?:${s}|([\\w-]+))(?:=(["']?)(?:${s}|((?:.?(?!["']?\\s+(?:\\S+)=|\\s*/>|\\s*[>"']))+.))?\\3)?`,"g");let o;for(;(o=a.exec(e))!==null;){const[,l,r,c,p,x]=o,u=!!c;let d=typeof l<"u"?t[parseInt(l,10)]:r,h=typeof p<"u"?t[parseInt(p,10)]:x;u&&typeof h>"u"&&(h=""),typeof h<"u"?n.push([d,h,u]):n.push([d])}return n},rt=["key","state","onCreate","onChange","onHydrate","onBeforeRecycle","onRecycle","onBeforeUpdate","onUpdate"];class f extends oe{constructor(t={}){super(...arguments),this.componentOptions=[],rt.forEach(n=>{n in t&&(this[n]=t[n],this.componentOptions.push(n))});const s={};Object.keys(t).forEach(n=>{this.viewOptions.indexOf(n)===-1&&this.componentOptions.indexOf(n)===-1&&(s[n]=t[n])}),this.props=new ie(s),this.options=t,this.partial=this.partial.bind(this),this.onChange=this.onChange.bind(this),this.onCreate.apply(this,arguments)}events(){const t={};return this.eventsManager.types.forEach(s=>{const n=f.ATTRIBUTE_EVENT(s,this.uid),a=function(o,l,r){const c=r.getAttribute(n);if(c){let p=this.eventsManager.listeners[parseInt(c,10)];typeof p=="string"&&(p=this[p]),ne(p),p.call(this,o,l,r)}};t[`${s} [${n}]`]=a,t[s]=a}),t}ensureElement(){if(this.eventsManager=new Je,this.template=J(this.template,this),this.el){if(this.el=J(this.el,this),!this.el.parentNode){const t=`Hydration failed in ${this.constructor.name}#${this.uid}`;throw new Error(t)}this.toString(),this.hydrate(this.el.parentNode)}}isContainer(){return this.template.elements.length===0&&this.template.interpolations.length===1}subscribe(t,s="change",n=this.onChange){return t.on&&this.listenTo(t,s,n),this}hydrate(t){return["model","state","props"].forEach(s=>{this[s]&&this.subscribe(this[s])}),this.isContainer()?(this.children[0].hydrate(t),this.el=this.children[0].el):(this.template.elements.forEach((s,n)=>{n===0?(s.hydrate(t),this.el=s.ref):s.hydrate(this.el)}),this.template.interpolations.forEach(s=>s.hydrate(this.el)),this.children.forEach(s=>s.hydrate(this.el))),this.delegateEvents(),this.onHydrate.call(this),this}recycle(t){if(this.onBeforeRecycle.call(this),t){const s=fe(t,f.MARKER_RECYCLED(this.uid),Ce);at(s,this.el)}return this}updateProps(t){return this.props.set(t),this.onRecycle.call(this),this}getRecycledMarker(){return`<!--${f.MARKER_RECYCLED(this.uid)}-->`}partial(t,...s){const n=ve(qe(be(Te(t,s).trim(),s),s),s).map(a=>G(a,this,"partial"));return new ge(n)}renderTemplatePart(t,s,n){const a=G(t,this,"template part");if(typeof a>"u"||a===null||a===!1||a===!0)return"";if(a instanceof we)return`${a}`;if(a instanceof f)return`${s(a,n)}`;if(a instanceof ge){if(a.items.length===1)return this.renderTemplatePart(a.items[0],s,n);n.push();const o=a.items.map(l=>(n.increment(),this.renderTemplatePart(l,s,n))).join("");return n.pop(),o}if(Array.isArray(a)){n.pause();const o=Be(a).map(l=>this.renderTemplatePart(l,s,n)).join("");return n.resume(),o}if(a instanceof ke){const o=a.tracker;o.reset();const l=this.isContainer()?"":`<!--${a.getStart()}-->`,r=this.isContainer()?"":`<!--${a.getEnd()}-->`;return`${l}${this.renderTemplatePart(a.expression,s,o)}${r}`}return`${f.sanitize(a)}`}toString(){this.destroyChildren(),this.eventsManager.reset();const t=(s,n)=>(n.track(s),this.addChild(s));return this.template.parts.map(s=>this.renderTemplatePart(s,t)).join("")}render(){if(this.destroyed)return this;if(!this.el){const n=ce(this);return this.hydrate(n),this}this.onBeforeUpdate.call(this),this.eventsManager.reset();const t=this.children;this.children=[];const s=[];return this.template.interpolations.forEach(n=>{const a=n.tracker;a.reset();const o=[],l=[],r=d=>{let h=d,b;return d.key?b=t.find(E=>E.key===d.key):b=a.findRecyclable(d),b?(h=b.getRecycledMarker(),l.push([b,d]),a.track(b)):(o.push(d),a.track(d)),h},c=this.renderTemplatePart(n.expression,r,a),p=([d,h],b)=>{s.push([d,h.props.toJSON()]),this.addChild(d).recycle(b),h.destroy()};if(a.hasSingleComponent()){p(l[0],null);return}const x=ce(c),u=d=>()=>{l.forEach(h=>p(h,d)),o.forEach(h=>this.addChild(h).hydrate(d))};this.isContainer()?n.updateElement(this.el,x,u(this.el.parentNode)):n.update(x,u(this.el))}),t.forEach(n=>{this.children.indexOf(n)<0&&n.destroy()}),s.forEach(([n,a])=>{n.updateProps(a)}),this.isContainer()?this.el=this.children[0].el:this.template.elements.forEach(n=>n.update()),this.eventsManager.hasPendingTypes()&&this.delegateEvents(),this.onUpdate.call(this),this}onCreate(){}onChange(){this.render()}onHydrate(){}onBeforeRecycle(){}onRecycle(){}onBeforeUpdate(){}onUpdate(){}onDestroy(){}static markAsSafeHTML(t){return new we(t)}static extend(t){const s=this;class n extends s{}return Object.assign(n.prototype,typeof t=="function"?t(s.prototype):t),n}static mount(t,s,n){const a=new this(t);return s&&(n?a.toString():s.append(ce(a)),a.hydrate(s)),a}static create(t,...s){typeof t=="function"&&(s=[t],t=["",""]);const n=null,a=[],o=[],l=ve(lt(ot(be(Te(t,s).trim(),s),s,a),s,o),s);return this.extend({source:n,template(){return{elements:a.map(r=>new Ge({getSelector:r.getSelector.bind(this),getAttributes:r.getAttributes.bind(this)})),interpolations:o.map(r=>new ke({getStart:r.getStart.bind(this),getEnd:r.getEnd.bind(this),expression:r.expression,shouldSkipFind:Ce,shouldSkipSync:nt})),parts:l}}})}}f.ATTRIBUTE_ELEMENT="data-rst-el";f.ATTRIBUTE_EVENT=(e,t)=>`data-rst-on-${e}-${t}`;f.DATASET_ELEMENT="rstEl";f.PLACEHOLDER=e=>`__RASTI_PLACEHOLDER_${e}__`;f.MARKER_RECYCLED=e=>`rst-r-${e}`;f.MARKER_START=e=>`rst-s-${e}`;f.MARKER_END=e=>`rst-e-${e}`;var y=f.create`<div></div>`;const Ee=e=>e!==null&&typeof e=="object"&&!Array.isArray(e),it=!1,ct=e=>e.replace(/([A-Z])/g,t=>`-${t[0].toLowerCase()}`),pe=(e,t)=>typeof e!="function"?e:e.call(t),dt=["prefix","generateUid","generateClassName","shouldAttachToDOM","attributes","renderers"];class T{constructor(t,s={}){this.preinitialize.apply(this,arguments),this.styles=t,this.classes={},dt.forEach(a=>{a in s&&(this[a]=s[a])}),this.renderers=this.renderers?pe(this.renderers,this).map(a=>typeof a=="string"?this[a]:a):[this.parseStyles,this.renderStyles],this.prefix=this.prefix?pe(this.prefix,this):T.prefix,this.uid=this.generateUid();let n=0;Object.keys(t).forEach(a=>{a.match(T.classRegex)&&(this.classes[a]=this.generateClassName(a,++n))})}preinitialize(){}generateUid(){const t=JSON.stringify(this.styles);let s=2166136261;for(let n=0;n<t.length;n++)s^=t.charCodeAt(n),s=s*16777619>>>0;return s.toString(36)}generateClassName(t,s){return`${this.prefix[0]}-${this.uid}-${s}`}render(){return this.renderers.reduce((t,s)=>s.call(this,t),this.styles)}renderStyles(t,s=1){return Object.keys(t).reduce((n,a)=>{const o=t[a];let l="",r="",c="";if(Ee(o)){if(Object.keys(o).length>0){const p=this.renderStyles(o,s+1);n.push(`${l}${a}${c}{${r}${p}${l}}${r}`)}}else typeof o<"u"&&o!==null&&n.push(`${l}${a}:${c}${o};${r}`);return n},[]).join("")}parseStyles(t,s,n,a){const o=c=>c in this.classes?`.${this.classes[c]}`:c,l=c=>a&&n?`${n} ${c}`:c.match(T.globalPrefixRegex)?`${n?`${n} `:""}${c.replace(T.globalPrefixRegex,"")}`:o(c).replace(T.referenceRegex,(p,x)=>o(x)).replace(T.nestedRegex,n);return Object.keys(t).reduce((c,p)=>{const x=t[p];if(Ee(x))if(p.match(T.globalRegex))Object.assign(s||c,this.parseStyles(x,c,n,!0));else if((p.match(T.nestedRegex)||p.match(T.globalPrefixRegex))&&s){const u=l(p);s[u]={},Object.assign(s[u],this.parseStyles(x,s,u))}else{const u=l(p);c[u]={};const d=u.match(/@/)?[]:[c,u];Object.assign(c[u],this.parseStyles(x,...d))}else typeof x<"u"&&x!==null&&(c[p.match(/-/)?p:ct(p)]=x);return c},{})}getAttributes(){const t=Object.assign({},pe(this.attributes,this));return t[`data-${this.prefix}-uid`]=this.uid,t}toString(){const t=this.getAttributes(),s=Object.keys(t).map(a=>` ${a}="${t[a]}"`).join(""),n="";return`<style${s}>${n}${this.render()}</style>${n}`}shouldAttachToDOM(){return typeof document<"u"&&!document.querySelector(`style[data-${this.prefix}-uid="${this.uid}"]`)}attach(){if(T.registry.some(({uid:t})=>t===this.uid)||T.registry.push(this),this.shouldAttachToDOM()){this.el=document.createElement("style");const t=this.getAttributes();Object.keys(t).forEach(s=>{this.el.setAttribute(s,t[s])}),this.el.textContent=this.render(),document.head.appendChild(this.el)}return this}destroy(){const t=T.registry.indexOf(this);return t>-1&&T.registry.splice(t,1),this.el&&(this.el.parentNode&&this.el.parentNode.removeChild(this.el),this.el=null),this}static toString(){return T.registry.join("")}static toCSS(){return T.registry.map(t=>t.render()).join("")}static destroy(){T.registry.slice().forEach(t=>t.destroy())}}T.classRegex=/^\w+$/;T.globalRegex=/^@global$/;T.globalPrefixRegex=/^@global\s+/;T.referenceRegex=/\$(\w+)/g;T.nestedRegex=/&/g;T.prefix="fun";T.indent="    ";T.registry=[];T.debug=it;const pt=(e,t)=>new T(e,t).attach();function I(e){return arguments.length>1?I(Array.from(arguments)):e?Array.isArray(e)?e.map(t=>I(t)).filter(Boolean).flat().join(" "):typeof e=="object"?I(Object.keys(e).filter(t=>!!e[t])):typeof e=="string"?e:"":""}const ht=(e,t)=>e.reduce((s,n,a)=>(Object.assign(s,t(n,a)),s),{}),le=e=>`@media (min-width: ${{sm:640,md:768,lg:1024,xl:1280,xxl:1536}[e]}px)`,O=e=>ht(["primary","secondary","neutral","error","warning","success"],e),A=pt,{classes:ut}=A({root:{display:"inline-flex",alignItems:"center",justifyContent:"space-evenly",boxSizing:"border-box",cursor:"pointer",borderRadius:"var(--rui-borderRadius-sm)",padding:"var(--rui-spacing-sm)",maxHeight:"100%",fontFamily:"var(--rui-typography-button-fontFamily)",fontWeight:"var(--rui-typography-button-fontWeight)",fontSize:"var(--rui-typography-button-fontSize)",lineHeight:"var(--rui-typography-button-lineHeight)",textTransform:"var(--rui-typography-button-textTransform)",textDecoration:"var(--rui-typography-button-textDecoration)",transition:["background-color 0.15s cubic-bezier(0.4, 0, 0.2, 1)","color 0.15s cubic-bezier(0.4, 0, 0.2, 1)","border-color 0.15s cubic-bezier(0.4, 0, 0.2, 1)","box-shadow 0.15s cubic-bezier(0.4, 0, 0.2, 1)","transform 0.15s cubic-bezier(0.4, 0, 0.2, 1)"].join(", "),"&:where([data-disabled])":{cursor:"not-allowed"},"&>svg:first-child":{padding:"0 var(--rui-spacing-xs) 0 0"},"&>svg:last-child":{padding:"0 0 0 var(--rui-spacing-xs)"},"&>svg:only-child":{padding:"0"},'&:where([data-size="sm"])':{fontSize:"var(--rui-fontSize-xs)"},'&:where([data-size="lg"])':{fontSize:"var(--rui-fontSize-xl)"},'&:where([data-shape="pill"])':{borderRadius:"var(--rui-borderRadius-full)"},'&:where([data-shape="circle"])':{borderRadius:"50%",aspectRatio:"1 / 1",maxHeight:"none",padding:"var(--rui-spacing-xs)",minWidth:"var(--rui-spacing-xxxxl)",minHeight:"var(--rui-spacing-xxxxl)",justifyContent:"center"},'&:where([data-shape="circle"]):where([data-size="sm"])':{padding:"var(--rui-spacing-xxs)",minWidth:"var(--rui-spacing-xxl)",minHeight:"var(--rui-spacing-xxl)"},'&:where([data-shape="circle"]):where([data-size="lg"])':{padding:"var(--rui-spacing-sm)",minWidth:"calc(var(--rui-spacing-xxxxl) + var(--rui-spacing-sm))",minHeight:"calc(var(--rui-spacing-xxxxl) + var(--rui-spacing-sm))"},...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"])`]:{border:`1px solid var(--rui-palette-${e}-main)`,color:`rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.95)`,backgroundColor:`var(--rui-palette-${e}-main)`,"&:hover":{color:`rgb(var(--rui-palette-${e}-rgb-contrastDark) / 0.95)`,backgroundColor:`var(--rui-palette-${e}-dark)`,borderColor:`var(--rui-palette-${e}-dark)`},"&:where(:not([data-disabled])):active":{backgroundColor:`var(--rui-palette-${e}-dark)`,borderColor:`var(--rui-palette-${e}-dark)`,color:`rgb(var(--rui-palette-${e}-rgb-contrastDark) / 0.95)`,boxShadow:"inset 0 1px 2px rgb(0 0 0 / 0.1)",transform:"translateY(0.5px)"},"&:where([data-disabled])":{color:`rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.6)`,backgroundColor:`var(--rui-palette-${e}-light)`,borderColor:`var(--rui-palette-${e}-light)`,boxShadow:"none",transform:"none","&:hover":{color:`rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.6)`,backgroundColor:`var(--rui-palette-${e}-light)`,borderColor:`var(--rui-palette-${e}-light)`},"&:active":{color:`rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.6)`,backgroundColor:`var(--rui-palette-${e}-light)`,borderColor:`var(--rui-palette-${e}-light)`,boxShadow:"none",transform:"none"}}}})),...O(e=>({[`&:where([data-variant="outlined"][data-color="${e}"])`]:{border:`1px solid var(--rui-palette-${e}-main)`,color:`var(--rui-palette-${e}-foregroundMain)`,backgroundColor:"transparent","&:hover":{backgroundColor:`rgb(var(--rui-palette-${e}-rgb-light) / 0.14)`},"&:where(:not([data-disabled])):active":{backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.12)`,transform:"translateY(0.5px)"},"&:where([data-disabled])":{color:`rgb(var(--rui-palette-${e}-rgb-foregroundLevel3) / 0.6)`,borderColor:`rgb(var(--rui-palette-${e}-rgb-light) / 0.6)`,transform:"none","&:hover":{color:`rgb(var(--rui-palette-${e}-rgb-foregroundLevel3) / 0.6)`,backgroundColor:"transparent"},"&:active":{transform:"none",backgroundColor:"transparent"}}}})),...O(e=>({[`&:where([data-variant="plain"][data-color="${e}"])`]:{border:"none",background:"transparent",color:`var(--rui-palette-${e}-foregroundMain)`,"&:hover":{color:`var(--rui-palette-${e}-foregroundDark)`,backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.05)`},"&:where(:not([data-disabled])):active":{color:`var(--rui-palette-${e}-foregroundDark)`,backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.07)`,transform:"translateY(0.5px)"},"&:where([data-disabled])":{color:`rgb(var(--rui-palette-${e}-rgb-foregroundLight) / 0.6)`,transform:"none","&:hover":{color:`rgb(var(--rui-palette-${e}-rgb-foregroundLight) / 0.6)`,backgroundColor:"transparent"},"&:active":{backgroundColor:"transparent",transform:"none"}}}})),...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"]):where(:not([data-disabled])):focus-visible`]:{outline:`2px solid rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.95)`,outlineOffset:"2px"},[`&:where([data-variant="outlined"][data-color="${e}"]):where(:not([data-disabled])):focus-visible`]:{outline:`2px solid var(--rui-palette-${e}-main)`,outlineOffset:"2px"},[`&:where([data-variant="plain"][data-color="${e}"]):where(:not([data-disabled])):focus-visible`]:{outline:`2px solid var(--rui-palette-${e}-main)`,outlineOffset:"2px"}})),'&:where([data-group]):where(:not([data-shape="circle"])):where(:not([data-shape="pill"])):not(:first-child)':{marginLeft:"-1px",...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"])`]:{borderLeftColor:`var(--rui-palette-${e}-dark)`}}))},'&:where([data-group]):where(:not([data-shape="circle"])):where(:not([data-shape="pill"])):not(:first-child):not(:last-child)':{borderRadius:"0"},'&:where([data-group]):where(:not([data-shape="circle"])):where(:not([data-shape="pill"])):first-child':{borderTopRightRadius:"0",borderBottomRightRadius:"0"},'&:where([data-group]):where(:not([data-shape="circle"])):where(:not([data-shape="pill"])):last-child':{borderTopLeftRadius:"0",borderBottomLeftRadius:"0"},"&:where([data-current])":{fontWeight:"var(--rui-fontWeight-xl)"},...O(e=>({[`&:where([data-variant="outlined"][data-color="${e}"][data-current])`]:{border:`1px solid var(--rui-palette-${e}-main)`,backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.05)`,color:`var(--rui-palette-${e}-foregroundDark)`},[`&:where([data-variant="outlined"][data-color="${e}"][data-current]):hover`]:{backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.08)`,border:`1px solid var(--rui-palette-${e}-main)`,color:`var(--rui-palette-${e}-foregroundDark)`},[`&:where([data-variant="plain"][data-color="${e}"][data-current])`]:{backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.03)`,color:`var(--rui-palette-${e}-foregroundDark)`},[`&:where([data-variant="plain"][data-color="${e}"][data-current]):hover`]:{backgroundColor:`rgb(var(--rui-palette-${e}-rgb-main) / 0.05)`,color:`var(--rui-palette-${e}-foregroundDark)`},[`&:where([data-variant="solid"][data-color="${e}"][data-current])`]:{color:`rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.98)`,backgroundColor:`var(--rui-palette-${e}-dark)`,border:`1px solid var(--rui-palette-${e}-dark)`,boxShadow:"none"},[`&:where([data-variant="solid"][data-color="${e}"][data-current]):hover`]:{color:`rgb(var(--rui-palette-${e}-rgb-contrastDark) / 0.95)`,backgroundColor:`var(--rui-palette-${e}-dark)`,borderColor:`var(--rui-palette-${e}-dark)`}}))}}),M=y.create`
    <${({props:e})=>e.href?"a":e.type?"input":"button"}
        class="${({props:e})=>I([ut.root,e.className])}"
...
`.extend({renderHeaderContent(){return this.props.renderChildren?this.props.renderChildren():this.partial`
            ${this.props.title?this.partial`<h2 class="${Z.title}" data-slot="title" id="${this.props.titleId||null}">${this.props.title}</h2>`:null}
            ${this.props.handleClose&&this.props.closeButton?this.partial`<${M}
        ...
        `}}),vt=y.create`
    <div class="${({props:e})=>I([Z.content,e.className])}" data-slot="content">
        ${({props:e})=>e.renderChildren()}
    </div>
`,bt=y.create`
    <div class="${({props:e})=>I([Z.footer,e.className])}" data-slot="footer">
        ${({props:e})=>e.renderChildren()}
    </div>
`,W=y.create`
    <div
        class="${({props:e})=>I([Z.root,e.className])}"
...
`.extend({onCreate(){this.handleDocumentKeydown=e=>{if(e.key==="Escape"&&this.props.handleClose){this.props.handleClose();return}if(e.key!=="Tab"||!this.dialogEl||!this.dialogEl.contains(document.activeElement))return;const t=Se(this.dialogEl);if(t.length===0){e.preventDefault();return}if(t.length===1){document.activeElement===t[0]&&e.preventDefault();return}const s=t[0],n=t[t.length-1];e.shiftKey?document.activeElement===s&&(e.preventDefault(),n.focus()):document.activeElement===n&&(e.preventDefault(),s.focus())},this.handleClickOutside=e=>{this.props.handleClose&&!this.el.contains(e.target)&&this.props.handleClose()}},onHydrate(){this.dialogEl=this.$('[role="dialog"]'),this._previousActiveElement=document.activeElement,this._panelTabindexSet=!1;const e=document.body.style.overflow;if(document.body.style.overflow="hidden",this._originalOverflow=e,document.addEventListener("keydown",this.handleDocumentKeydown),this.dialogEl){const t=Se(this.dialogEl);t.length>0?t[0].focus():(this.dialogEl.setAttribute("tabindex","-1"),this._panelTabindexSet=!0,this.dialogEl.focus())}setTimeout(()=>{document.addEventListener("click",this.handleClickOutside)},0)},onDestroy(){if(this._panelTabindexSet&&this.dialogEl&&this.dialogEl.removeAttribute("tabindex"),this._originalOverflow!==void 0&&(document.body.style.overflow=this._originalOverflow),document.removeEventListener("keydown",this.handleDocumentKeydown),document.removeEventListener("click",this.handleClickOutside),this._previousActiveElement&&typeof this._previousActiveElement.focus=="function")try{this._previousActiveElement.focus()}catch{}}});W.Header=ft;W.Content=vt;W.Footer=bt;var F={},Le;function yt(){if(Le)return F;Le=1,Object.defineProperty(F,"__esModule",{value:!0}),F.PathError=F.TokenData=void 0,F.parse=p,F.compile=x,F.match=h,F.pathToRegexp=b,F.stringify=z;const e="/",t=i=>i,s=/^[$_\p{ID_Start}]$/u,n=/^[$\u200c\u200d\p{ID_Continue}]$/u,a=/^[$_\p{ID_Start}][$\u200c\u200d\p{ID_Continue}]*$/u;function o(i){return i.replace(/[{}()\[\]+?!:*\\]/g,"\\$&")}function l(i){return i.replace(/[.+*?^${}()[\]|/\\]/g,"\\$&")}class r{constructor(m,j){this.tokens=m,this.originalPath=j}}F.TokenData=r;class c extends TypeError{constructor(m,j){let v=m;j&&(v+=`: ${j}`),v+="; visit https://git.new/pathToRegexpError for info",super(v),this.originalPath=j}}F.PathError=c;function p(i,m={}){const{encodePath:j=t}=m,v=[...i];let g=0;function C(_){const k=[];let $="";function D(){$&&(k.push({type:"text",value:j($)}),$="")}for(;g<v.length;){const L=v[g++];if(L===_)return D(),k;if(L==="\\"){if(g===v.length)throw new c(`Unexpected end after \\ at index ${g}`,i);$+=v[g++];continue}if(L===":"||L==="*"){const U=L===":"?"param":"wildcard";let w="";if(s.test(v[g]))do w+=v[g++];while(n.test(v[g]));else if(v[g]==='"'){let B=g;for(;g<v.length;){if(v[++g]==='"'){g++,B=0;break}v[g]==="\\"&&g++,w+=v[g]}if(B)throw new c(`Unterminated quote at index ${B}`,i)}if(!w)throw new c(`Missing parameter name at index ${g}`,i);D(),k.push({type:U,name:w});continue}if(L==="{"){D(),k.push({type:"group",tokens:C("}")});continue}if(L==="}"||L==="("||L===")"||L==="["||L==="]"||L==="+"||L==="?"||L==="!")throw new c(`Unexpected ${L} at index ${g-1}`,i);$+=L}if(_)throw new c(`Unexpected end at index ${g}, expected ${_}`,i);return D(),k}return new r(C(""),i)}function x(i,m={}){const{encode:j=encodeURIComponent,delimiter:v=e}=m,g=typeof i=="object"?i:p(i,m),C=u(g.tokens,v,j);return function(k={}){const $=[],D=C(k,$);if($.length)throw new TypeError(`Missing parameters: ${$.join(", ")}`);return D}}function u(i,m,j){const v=i.map(g=>d(g,m,j));return(g,C)=>{let _="";for(const k of v)_+=k(g,C);return _}}function d(i,m,j){if(i.type==="text")return()=>i.value;if(i.type==="group"){const g=u(i.tokens,m,j);return(C,_)=>{const k=_.length,$=g(C,_);return _.length===k?$:(_.length=k,"")}}const v=j||t;return i.type==="wildcard"&&j!==!1?(g,C)=>{const _=g[i.name];if(_==null)return C.push(i.name),"";if(!Array.isArray(_)||_.length===0)throw new TypeError(`Expected "${i.name}" to be a non-empty array`);let k="";for(let $=0;$<_.length;$++){if(typeof _[$]!="string")throw new TypeError(`Expected "${i.name}/${$}" to be a string`);$>0&&(k+=m),k+=v(_[$])}return k}:(g,C)=>{const _=g[i.name];if(_==null)return C.push(i.name),"";if(typeof _!="string")throw new TypeError(`Expected "${i.name}" to be a string`);return v(_)}}function h(i,m={}){const{decode:j=decodeURIComponent,delimiter:v=e}=m,{regexp:g,keys:C}=b(i,m),_=C.map(k=>j===!1?t:k.type==="param"?j:$=>$.split(v).map(j));return function($){const D=g.exec($);if(!D)return!1;const L=D[0],U=Object.create(null);for(let w=1;w<D.length;w++){if(D[w]===void 0)continue;const B=C[w-1],Y=_[w-1];U[B.name]=Y(D[w])}return{path:L,params:U}}}function b(i,m={}){const{delimiter:j=e,end:v=!0,sensitive:g=!1,trailing:C=!0}=m,_=[];let k="",$=0;function D(U){if(Array.isArray(U)){for(const B of U)D(B);return}const w=typeof U=="object"?U:p(U,m);E(w.tokens,0,[],B=>{if($>=256)throw new c("Too many path combinations",w.originalPath);$>0&&(k+="|"),k+=N(B,j,_,w.originalPath),$++})}D(i);let L=`^(?:${k})`;return C&&(L+="(?:"+l(j)+"$)?"),L+=v?"$":"(?="+l(j)+"|$)",{regexp:new RegExp(L,g?"":"i"),keys:_}}function E(i,m,j,v){for(;m<i.length;){const g=i[m++];if(g.type==="group"){const C=j.length;E(g.tokens,0,j,_=>E(i,m,_,v)),j.length=C;continue}j.push(g)}v(j)}function N(i,m,j,v){let g="",C="",_="",k=0,$=0,D=0;function L(w,B){for(;w<i.length;){const Y=i[w++];if(Y.type===B)return!0;if(Y.type==="text"&&Y.value.includes(m))break}return!1}function U(w){let B="";for(;w<i.length;){const Y=i[w++];if(Y.type!=="text")break;B+=Y.value}return B}for(;D<i.length;){const w=i[D++];if(w.type==="text"){g+=l(w.value),C+=w.value,k===2&&(_+=w.value),w.value.includes(m)&&($=0);continue}if(w.type==="param"||w.type==="wildcard"){if(k&&!C)throw new c(`Missing text before "${w.name}" ${w.type}`,v);w.type==="param"?(g+=$&2?`(${R(m,C)}+)`:L(D,"wildcard")?`(${R(m,U(D))}+)`:$&1?`(${R(m,C)}+|${l(C)})`:`(${R(m,"")}+)`,$|=k=1):(g+=$&2?`(${R(C,"")}+)`:_?`(${R(_,"")}+|${R(m,"")}+)`:"([^]+)",_="",$|=k=2),j.push(w),C="";continue}throw new TypeError(`Unknown token type: ${w.type}`)}return g}function R(i,m){return m.length>i.length?R(m,i):(i===m&&(m=""),m.length>1?`(?:(?!${l(i)}|${l(m)})[^])`:i.length>1?`(?:(?!${l(i)})[^${l(m)}])`:`[^${l(i+m)}]`)}function V(i,m){let j="";for(;m<i.length;){const v=i[m++];if(v.type==="text"){j+=o(v.value);continue}if(v.type==="group"){j+="{"+V(v.tokens,0)+"}";continue}if(v.type==="param"){j+=":"+K(v.name,i[m]);continue}if(v.type==="wildcard"){j+="*"+K(v.name,i[m]);continue}throw new TypeError(`Unknown token type: ${v.type}`)}return j}function z(i){return V(i.tokens,0)}function K(i,m){return!a.test(i)||m?.type==="text"&&n.test(m.value[0])?JSON.stringify(i):i}return F}var Me=yt();function jt(e,t={}){const{baseUrl:s=""}=t,n=u=>{const[d,h]=u.replace(s,"").split("?");return{pathname:d,query:h}},a=(u,d)=>Me.match(d,{decode:decodeURIComponent})(u),o=u=>{const{pathname:d,query:h}=n(u);for(const b of e){const E=a(d,b.path);if(E){const N={path:b.path,params:x(E.params),query:Object.fromEntries(new URLSearchParams(h).entries()),test:R=>!!a(n(R).pathname,b.path)};return()=>b.action(N)}}return null},l=(u,d={})=>{const{addToHistory:h=!0,replaceHistory:b=!1}=d,E=o(u);E?(h&&typeof window<"u"&&window.history[b?"replaceState":"pushState"]({},"",u),E()):console.error("No route matched:",u)},r=(u,d={},h={})=>{const E=Me.compile(u,{encode:encodeURIComponent})(d),N=new URLSearchParams(h).toString();return`${s}${N?`${E}?${N}`:E}`},c=u=>{const d=h=>{if(h.defaultPrevented||h.button!==0||h.metaKey||h.ctrlKey||h.shiftKey||h.altKey)return;const b=h.target.closest("a[data-router]");if(b&&b.href){h.preventDefault();const E=new URL(b.href);window.scrollTo({top:0,behavior:"instant"}),l(E.pathname+E.search)}};return u.addEventListener("click",d),()=>{u.removeEventListener("click",d)}},p=()=>{const u=()=>{l(window.location.pathname+window.location.search,{addToHistory:!1})};return window.addEventListener("popstate",u),()=>{window.removeEventListener("popstate",u)}},x=u=>{const d={};for(const[h,b]of Object.entries(u))d[h]=String(b).replace(/[<>]/g,"");return d};return{navigate:l,createUrl:r,delegateNavigation:c,bindHistory:p}}const _t=e=>({fontFamily:`var(--rui-typography-${e}-fontFamily)`,fontWeight:`var(--rui-typography-${e}-fontWeight)`,fontSize:`var(--rui-typography-${e}-fontSize)`,lineHeight:`var(--rui-typography-${e}-lineHeight)`}),wt=["h1","h2","h3","h4","titleLg","titleMd","titleSm","bodyLg","bodyMd","bodySm","caption"],{classes:xt}=A({root:{color:"var(--rui-palette-neutral-foregroundLevel1)",margin:"var(--rui-spacing-md) 0",...Object.fromEntries(wt.map(e=>[`&:where([data-level="${e}"])`,_t(e)]))}}),Ae=({props:e})=>{switch(e.level){case"h1":return"h1";case"h2":return"h2";case"h3":return"h3";case"h4":return"h4";case"titleLg":return"h2";case"titleMd":return"h2";case"titleSm":return"h2";case"caption":return"caption";default:return"p"}},S=y.create`
    <${Ae}
        class="${({props:e})=>I([xt.root,e.className])}"
...
`,{classes:Re}=A({"@global":{html:{scrollBehavior:"smooth"},body:{margin:0,backgroundColor:"var(--rui-palette-neutral-backgroundLevel1)",fontFamily:"var(--rui-fontFamily-body)"},"a.anchor, h2":{scrollMarginTop:"calc(var(--rui-app-appBarHeight) + var(--rui-spacing-xs))"}},root:{},appBarMenuDialog:{},[le("sm")]:{$appBarMenuDialog:{display:"none"}}}),Ct=e=>{const{title:t,AppBar:s,AppBarMenuContent:n,Cover:a,Description:o,Features:l,Readme:r,AboutSite:c,Api:p,Footer:x}=e;return y.create`
        <div class="${Re.root}">
            <${s} 
            ...
            ${({state:d,partial:h})=>d.menuOpen&&n?h`<${W}
                ...
                >
                    ...
                    />
                    <${W.Content}
                        renderChildren=${()=>n.mount({handleOpen:b=>{d.menuOpen=b},location:d.location})}
                    />
                </${W}>`:null}

            ${({state:d,partial:h})=>d.location.params.notFound?h`<${kt} />`:d.location.test("/api/")?h`<${p} />`:h`
                        <${a} />
                        ${o?h`<${o} />`:null}
                        <${l} />
                        <${r} />
    ...
    `.extend({onCreate(d={}){this.state=new ie({location:null,menuOpen:!1});const h=[{path:"/api/",action:R=>this.updateRoute(R)},{path:"/",action:R=>this.updateRoute(R)},{path:"*notFound",action:R=>this.updateRoute(R)}];this.router=jt(h);const b=typeof window<"u"?window.location.pathname+window.location.search:null,E=d.url??b??"/",N=b!==null&&E!==b;this.router.navigate(E,N?{replaceHistory:!0}:{addToHistory:!1})},onHydrate(){this.router&&this.destroyQueue.push(this.router.delegateNavigation(this.el),this.router.bindHistory())},updateRoute(d){this.state.location=d,typeof document<"u"&&(document.title=this.getTitle())},getTitle(){const d=this.state.location.params.notFound?" - Not Found":this.state.location.test("/api/")?" - API Documentation":this.state.location.test("/")?" - Home":"";return`${t}${d}`}})},{classes:Q}=A({section:{display:"flex",alignItems:"center",gap:"var(--rui-spacing-sm)"},left:{flex:"0 0 auto",justifyContent:"flex-start"},center:{flex:"1 1 auto",justifyContent:"center"},right:{flex:"0 0 auto",justifyContent:"flex-end"}}),Tt=y.create`
    <div class="${({props:e})=>I([Q.section,Q.left,e.className])}" data-slot="left">
        ${({props:e})=>e.renderChildren()}
    </div>
`,Et=y.create`
    <div class="${({props:e})=>I([Q.section,Q.center,e.className])}" data-slot="center">
        ${({props:e})=>e.renderChildren()}
    </div>
`,St=y.create`
    <div class="${({props:e})=>I([Q.section,Q.right,e.className])}" data-slot="right">
        ${({props:e})=>e.renderChildren()}
    </div>
`,{classes:Lt}=A({root:{position:"sticky",top:0,zIndex:"var(--rui-zIndex-appBar, 1000)",display:"flex",alignItems:"center",minHeight:"56px",padding:"var(--rui-spacing-sm) var(--rui-spacing-md)",gap:"var(--rui-spacing-md)",borderBottom:"1px solid rgba(var(--rui-palette-neutral-rgb-foregroundSoftLevel3) / 0.5)",transition:"background-color 0.2s ease, border-color 0.2s ease",backgroundColor:"var(--rui-palette-neutral-backgroundLevel1)",...O(e=>({[`&:where([data-variant="outlined"][data-color="${e}"])`]:{borderBottomColor:`var(--rui-palette-${e}-foregroundSoftLevel3)`}})),...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"])`]:{backgroundColor:`var(--rui-palette-${e}-main)`,borderBottomColor:`var(--rui-palette-${e}-main)`,color:`var(--rui-palette-${e}-contrastMain)`},[`&:where([data-variant="solid"][data-color="${e}"]) *`]:{color:`var(--rui-palette-${e}-contrastMain)`}}))}}),q=y.create`
    <header
        class="${({props:e})=>I([Lt.root,e.className])}"
        data-variant="${({props:e})=>e.variant||"outlined"}"
        data-color="${({props:e})=>e.color||"neutral"}"
...
`;q.Left=Tt;q.Center=Et;q.Right=St;var Mt=y.create`
    <svg class="${({props:e})=>e.className||""}" width="${({props:e})=>e.width||"24"}" height="${({props:e})=>e.height||"24"}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon">
...
`;const{classes:ee}=A({root:{display:"flex",justifyContent:"right",alignItems:"center",height:"60px","& ul":{padding:0}},hiddenIfLight:{display:"var(--rui-app-hiddenIfLight)"},hiddenIfDark:{display:"var(--rui-app-hiddenIfDark)"}}),Dt=y.create`
    <div class="${ee.root}">
        <ul>
...
`,{classes:P}=A({root:{boxSizing:"border-box",height:"var(--rui-app-appBarHeight)",backgroundColor:"var(--rui-palette-neutral-backgroundLevel2)",position:"fixed",top:0,left:0,right:0,zIndex:1e3},leftContent:{display:"flex",alignItems:"center"},rightContent:{display:"flex",alignItems:"center",gap:"var(--rui-spacing-sm)"},navLinks:{display:"none","& ul":{display:"flex",justifyContent:"center",alignItems:"center",listStyle:"none",padding:0,margin:0,gap:"var(--rui-spacing-sm)","& li":{margin:0}}},menuButton:{display:"block"},icon:{width:"24px",height:"24px",fill:"var(--rui-palette-neutral-main)","a:hover &":{fill:"var(--rui-palette-neutral-dark)"}},hiddenIfLight:{display:"var(--rui-app-hiddenIfLight)"},hiddenIfDark:{display:"var(--rui-app-hiddenIfDark)"},logoInactive:{opacity:.5},menuContent:{"& nav":{maxWidth:"100%",display:"flex",justifyContent:"center",alignItems:"center","& ul":{display:"flex",justifyContent:"center",flexDirection:"column",alignItems:"center",listStyle:"none",padding:0,"& li":{margin:"var(--rui-spacing-md)"}}}},[le("sm")]:{$navLinks:{display:"block"},$menuButton:{display:"none"}}}),Ht=e=>{const{logoAlt:t,playgroundUrl:s,githubUrl:n,npmUrl:a}=e,o=y.create`
                ...
                ${({props:r,partial:c})=>c`
                    ...
                    <li>
                        <${M}
                            ...
                            variant="plain"
                            size="lg"
                            renderLeftIcon=${()=>ye.mount({className:P.icon})}
                        />
                    </li>
                    <li>
                        <${M}
                            ...
                            variant="plain"
                            size="lg"
                            renderLeftIcon=${()=>De.mount({className:P.icon})}
                        />
                    </li>
                `}
            </ul></nav>
        ...
        <${q} className="${P.root}">
            <${q.Left}>
                <div class="${P.leftContent}">
                    <a href="/" class="${({props:r})=>r.location.params.notFound||!r.location.test("/")?P.logoInactive:""}" aria-current="${({props:r})=>r.location.test("/")?"page":null}" data-router>
                        ...
                        <ul>
                            <li>
                                <${M}
                                    ...
                                    label="API"
                                    variant="plain"
                                    current="${({props:r})=>!r.location.params.notFound&&r.location.test("/api/")}"
                                />
                            </li>
                            <li>
                                <${M}
                            ...
                            <li>
                                <${M}
                                    ...
                                    variant="plain"
                                    shape="circle"
                                    renderChildren=${()=>ye.mount({className:P.icon})}
                                />
                            </li>
                            <li>
                                <${M}
                                    ...
                                    variant="plain"
                                    shape="circle"
                                    renderChildren=${()=>De.mount({className:P.icon})}
                                />
                            </li>
                        </ul>
                    </nav>
    ...
    `,AppBarMenuContent:o}},{classes:Ot}=A({root:{borderRadius:"var(--rui-borderRadius-md)",padding:"var(--rui-spacing-md)",backgroundColor:"var(--rui-palette-neutral-backgroundLevel2)",fontFamily:"var(--rui-fontFamily-body)",fontSize:"var(--rui-fontSize-bodyMd)",...O(e=>({[`&:where([data-color="${e}"])`]:{color:`var(--rui-palette-${e}-foregroundMain)`}})),...O(e=>({[`&:where([data-variant="outlined"][data-color="${e}"])`]:{border:`1px solid rgb(var(--rui-palette-${e}-rgb-level1) / 0.4)`}})),...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"])`]:{backgroundColor:`var(--rui-palette-${e}-main)`,color:`var(--rui-palette-${e}-contrastMain)`}})),'&:where([data-shadow="xs"])':{boxShadow:"var(--rui-shadow-xs)"},'&:where([data-shadow="sm"])':{boxShadow:"var(--rui-shadow-sm)"},'&:where([data-shadow="md"])':{boxShadow:"var(--rui-shadow-md)"},'&:where([data-shadow="lg"])':{boxShadow:"var(--rui-shadow-lg)"},'&:where([data-shadow="xl"])':{boxShadow:"var(--rui-shadow-xl)"},...O(e=>({[`&:where([data-interactive="true"][data-variant="solid"][data-color="${e}"]):focus-visible`]:{outline:`2px solid rgb(var(--rui-palette-${e}-rgb-contrastMain) / 0.95)`,outlineOffset:"2px"},[`&:where([data-interactive="true"][data-variant="outlined"][data-color="${e}"]):focus-visible`]:{outline:`2px solid var(--rui-palette-${e}-main)`,outlineOffset:"2px"},[`&:where([data-interactive="true"][data-variant="plain"][data-color="${e}"]):focus-visible`]:{outline:`2px solid var(--rui-palette-${e}-main)`,outlineOffset:"2px"}}))}}),H=y.create`
    <${({props:e})=>e.tag||"div"}
        class="${({props:e})=>I([Ot.root,e.className])}"
        data-variant="${({props:e})=>e.variant||"outlined"}"
        data-color="${({props:e})=>e.color||"neutral"}"
        data-shadow="${({props:e})=>e.shadow||!1}"
...
`,{classes:te}=A({root:{maxWidth:"var(--rui-app-maxWidth)",margin:"0 auto var(--rui-spacing-xxxxl)",padding:"var(--rui-spacing-lg)",color:"var(--rui-palette-neutral-foregroundLevel2)"},panel:{display:"flex",flexDirection:"column",alignItems:"flex-start",padding:"var(--rui-spacing-xl)","@global a":{color:"var(--rui-palette-primary-foregroundMain)",textDecoration:"none","&:visited, &:active":{color:"var(--rui-palette-primary-foregroundMain)"},"&:hover":{color:"var(--rui-palette-primary-foregroundDark)",textDecoration:"underline"}}},text:{margin:"0 0 var(--rui-spacing-lg)"},lastText:{margin:0}}),It=e=>{const{projectName:t,projectGithubUrl:s}=e;return y.create`
        <section id="how-this-site-is-built" class="${te.root}">
            <${H} className="${te.panel}" variant="outlined" shadow="xs">
                ...
                <${S} level="bodyMd" className="${te.text}">
                ...
                <${S} level="bodyMd" className="${te.lastText}">
    ...
    `},{classes:ue}=A({root:{display:"flex",justifyContent:"center",alignItems:"center",flexDirection:"column",padding:"var(--rui-spacing-xl) 0 var(--rui-spacing-xxxxl) 0",borderTop:"1px solid rgb(var(--rui-palette-neutral-rgb-foregroundLevel1) / 0.2)",backgroundColor:"var(--rui-palette-neutral-backgroundLevel2)","@global":{a:{color:"var(--rui-palette-neutral-main)",textDecoration:"none","&:visited, &:active":{color:"var(--rui-palette-neutral-main)"},"&:hover":{textDecoration:"underline",color:"var(--rui-palette-neutral-dark)"}}}},text:{color:"var(--rui-palette-neutral-foregroundLevel3)"}}),zt=e=>{const{licenseUrl:t,startYear:s}=e,n=()=>{const o=new Date().getFullYear();return o===s?o:`${s}-${o}`};return y.create`
        <footer class="${ue.root}">
            <${S} level="titleMd" className="${ue.text}">
                Released under the <a href="${t}" target="_blank">MIT License</a>
            </${S}>
            <${S} level="titleSm" className="${ue.text}">
                Copyright © ${n} by <a href="https://github.com/8tentaculos/" target="_blank">8tentaculos</a>
            </${S}>
        </footer>
    `},{classes:se}=A({root:{display:"flex",justifyContent:"center",alignItems:"center",height:"var(--rui-viewport-height)",flexDirection:"column",backgroundColor:"var(--rui-palette-neutral-backgroundLevel3)",boxShadow:"var(--rui-shadow-xs)",padding:"0 var(--rui-spacing-xl)",overflow:"hidden","@global":{h1:{textAlign:"center",margin:"var(--rui-app-appBarHeight) 0 0 0"},h2:{color:"var(--rui-palette-neutral-foregroundLevel2)",marginTop:0,marginBottom:"var(--rui-spacing-sm)"},h4:{color:"var(--rui-palette-neutral-foregroundLevel3)",marginBottom:"var(--rui-spacing-md)"},"h1 img":{width:"90%"},pre:{maxWidth:"100%"},code:{borderRadius:"var(--rui-borderRadius-sm)",boxShadow:"var(--rui-shadow-xs)",display:"block",background:"#282c34",color:"#abb2bf",overflowX:"auto",padding:"1em"}}},buttons:{"& a":{margin:"var(--rui-spacing-md) var(--rui-spacing-xs)"},display:"flex",justifyContent:"center"},[le("sm")]:{"$root h1":{margin:"var(--rui-app-appBarHeight) 0 0 0"},"$root h2":{marginBottom:"var(--rui-spacing-xl)"},"$root h4":{marginBottom:"var(--rui-spacing-xxl)"},"$root h1 img":{width:"75%"},"$buttons a":{margin:"var(--rui-spacing-xxxl) var(--rui-spacing-lg)"}},icon:{width:"24px",height:"24px",fill:"var(--rui-palette-secondary-main)"},hiddenIfLight:{display:"var(--rui-app-hiddenIfLight)"},hiddenIfDark:{display:"var(--rui-app-hiddenIfDark)"}}),Bt=e=>{const{logoAlt:t,tagline:s,renderSubtitle:n,CoverCodeExample:a,githubUrl:o}=e;return y.create`
        <section class="${se.root}">
            ...
            ${({partial:r})=>n?r`<${S} level="h4">${n}</${S}>`:null}
            ...
            <div class="${se.buttons}">
                ...
                <${M} 
                    ...
                    href="${o}"
                    target="_blank"
                    renderLeftIcon=${()=>ye.mount({className:se.icon})}
                />
            </div>
        </section>
    `},Pt=y.create`
...
`,Nt=Bt({logoAlt:"Rasti",tagline:"Modern MVC for building user interfaces",CoverCodeExample:Pt,githubUrl:"https://github.com/8tentaculos/rasti"}),{classes:Ut}=A({root:{display:"flex",justifyContent:"center",alignItems:"center",flexDirection:"column",padding:"var(--rui-spacing-lg)",margin:"0 auto",maxWidth:"var(--rui-app-maxWidth)"}}),Ft=y.create`
    <div class="${Ut.root}">
        <${S} level="h3">
...
<h3 id="using-es-modules-via-cdn">Using ES modules via CDN</h3>
...
<h3 id="using-a-umd-build-via-cdn">Using a UMD build via CDN</h3>
...
<h3 id="create-a-component">Create a <code>Component</code></h3>
...
<h3 id="adding-sub-components">Adding sub components</h3>
...
<h2 id="why-choose-rasti">Why Choose <strong>Rasti</strong>?</h2>
...
<p>The fastest way to start a real-world <strong>Rasti</strong> project is <a target="_blank" href="https://github.com/8tentaculos/create-rasti"><code>create-rasti</code></a>, the official scaffolding tool. It generates a ready-to-use <strong>Rasti</strong> + <strong>Vite</strong> setup with optional server-side rendering, routing, styling, and icon components.</p>
<pre><code class="bash language-bash"><span class="hljs-comment"># Interactive setup</span>
npm create rasti

<span class="hljs-comment"># Non-interactive single-page app</span>
npm create rasti my-app

<span class="hljs-comment"># Server-side rendering with routing and Tailwind CSS</span>
npm create rasti my-app --ssr --router --tailwind</code></pre>
<p>Available options include:</p>
<ul>
<li><strong>Rendering</strong> — Single-page app (default), server-side rendering (<code>--ssr</code>), or static pre-rendering (<code>--static</code>).</li>
<li><strong>Styling</strong> — Plain CSS (default), Tailwind CSS (<code>--tailwind</code>), or CSSFUN with light/dark theme support (<code>--cssfun</code>).</li>
<li><strong>Routing</strong> (<code>--router</code>) — A small universal router built on <code>path-to-regexp</code>.</li>
<li><strong>Icons</strong> (<code>--icons</code>) — Generate <strong>Rasti</strong> components from popular SVG icon sets (heroicons, akar-icons, feathericon, pixelarticons, and more).</li>
</ul>
<p>See the <a target="_blank" href="https://github.com/8tentaculos/create-rasti"><code>create-rasti</code> repository</a> for the full list of templates and options.</p>
<h2 id="example">Example</h2>
<p>To see how <strong>Rasti</strong>'s API and architecture come together in a small app, explore the sample <strong>TODO application</strong> in the <a target="_blank" href="https://github.com/8tentaculos/rasti/tree/master/example/todo">example folder</a> of the <strong>Rasti</strong> <a target="_blank" href="https://github.com/8tentaculos/rasti">GitHub repository</a>. It's a concise, self-contained reference for understanding how models, views, and components fit together in a simple application. Try it live <a target="_blank" href="https://rasti.js.org/example/todo/index.html">here</a>.</p>
<p>To scaffold a real-world project, use <a href="#scaffolding-a-new-project"><code>create-rasti</code></a>.</p>
...
<p><strong>Rasti</strong> ships with TypeScript declarations out of the box. The types are bundled in the package and resolved automatically.</p>
<h3 id="components">Components</h3>
<p>Pass generics explicitly to type the resulting class:</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">const</span> <span class="hljs-title class_">Header</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span>&lt;\{ <span class="hljs-attr">handleAddTodo</span>: <span class="hljs-function">(<span class="hljs-params"><span class="hljs-attr">title</span>: <span class="hljs-built_in">string</span></span>) =&gt;</span> <span class="hljs-built_in">void</span> \}&gt;<span class="hljs-string">\`
    &lt;header&gt;...&lt;/header&gt;
\`</span>;

<span class="hljs-keyword">new</span> <span class="hljs-title class_">Header</span>(\{ <span class="hljs-attr">handleAddTodo</span>: <span class="hljs-function">(<span class="hljs-params">t</span>) =&gt;</span> <span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(t) \}); <span class="hljs-comment">// ✅</span>

<span class="hljs-comment">// With a typed model:</span>
<span class="hljs-keyword">const</span> <span class="hljs-title class_">App</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span>&lt;\{\}, <span class="hljs-built_in">any</span>, <span class="hljs-title class_">AppModel</span>&gt;<span class="hljs-string">\`&lt;main&gt;...&lt;/main&gt;\`</span>;
<span class="hljs-title class_">App</span>.<span class="hljs-title function_">mount</span>(\{ <span class="hljs-attr">model</span>: <span class="hljs-keyword">new</span> <span class="hljs-title class_">AppModel</span>() \}, <span class="hljs-variable language_">document</span>.<span class="hljs-property">body</span>);</code></pre>
<p>Without generics, <code>Component.create</code> stays permissive (parity with JS):</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">const</span> <span class="hljs-title class_">Plain</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span><span class="hljs-string">\`&lt;div&gt;&lt;/div&gt;\`</span>;
<span class="hljs-keyword">new</span> <span class="hljs-title class_">Plain</span>(\{ <span class="hljs-attr">anything</span>: <span class="hljs-string">&#x27;goes&#x27;</span> \}); <span class="hljs-comment">// ✅</span></code></pre>
<p>When a component is used with inner content (<code>&lt;\$\{Card\}&gt;...&lt;/\$\{Card\}&gt;</code>), rasti injects a <code>renderChildren</code> function into its props at runtime. Declare it in <code>P</code> to use it:</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">const</span> <span class="hljs-title class_">Card</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span>&lt;\{ <span class="hljs-attr">title</span>: <span class="hljs-built_in">string</span>; <span class="hljs-attr">renderChildren</span>?: <span class="hljs-function">() =&gt;</span> <span class="hljs-built_in">any</span> \}&gt;<span class="hljs-string">\`
    &lt;div class=&quot;card&quot;&gt;
        &lt;h2&gt;<span class="hljs-subst">\$\{(\{ props \}) =&gt; props.title\}</span>&lt;/h2&gt;
        <span class="hljs-subst">\$\{(\{ props \}) =&gt; props.renderChildren?.()\}</span>
    &lt;/div&gt;
\`</span>;</code></pre>
<p><code>Component.extend</code> adds the object members to the instance type. Inside its methods, <code>this</code> is the extended component, and lifecycle overrides get their parameters typed automatically:</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">const</span> <span class="hljs-title class_">Counter</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span>&lt;\{ <span class="hljs-attr">initial</span>: <span class="hljs-built_in">number</span> \}&gt;<span class="hljs-string">\`&lt;div&gt;...&lt;/div&gt;\`</span>.<span class="hljs-title function_">extend</span>(\{
    <span class="hljs-title function_">onCreate</span>(<span class="hljs-params"></span>) \{
        <span class="hljs-variable language_">this</span>.<span class="hljs-property">state</span> = <span class="hljs-keyword">new</span> <span class="hljs-title class_">Model</span>(\{ <span class="hljs-attr">count</span>: <span class="hljs-variable language_">this</span>.<span class="hljs-property">props</span>.<span class="hljs-property">initial</span> \}); <span class="hljs-comment">// \`this\` is typed</span>
    \},
    <span class="hljs-title function_">onChange</span>(<span class="hljs-params">model, changed</span>) \{ <span class="hljs-comment">// parameters typed automatically</span>
        <span class="hljs-keyword">if</span> (<span class="hljs-string">&#x27;count&#x27;</span> <span class="hljs-keyword">in</span> changed) <span class="hljs-variable language_">this</span>.<span class="hljs-title function_">render</span>();
    \},
    <span class="hljs-title function_">increment</span>(<span class="hljs-params"></span>) \{ <span class="hljs-variable language_">this</span>.<span class="hljs-property">state</span>.<span class="hljs-property">count</span>++; \},
\});

<span class="hljs-title class_">Counter</span>.<span class="hljs-title function_">mount</span>(\{ <span class="hljs-attr">initial</span>: <span class="hljs-number">0</span> \}, <span class="hljs-variable language_">document</span>.<span class="hljs-property">body</span>).<span class="hljs-title function_">increment</span>(); <span class="hljs-comment">// ✅ increment is typed</span></code></pre>
<p>To read attributes off a typed <code>state</code> (or <code>model</code>) directly, define it as a named <code>Model</code> subclass with declaration merging and pass it as the <code>S</code> (or <code>M</code>) generic — then there are no casts anywhere:</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">class</span> <span class="hljs-title class_">ScoreState</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_ inherited__">Model</span>&lt;\{ <span class="hljs-attr">points</span>: <span class="hljs-built_in">number</span> \}&gt; \{\}
<span class="hljs-keyword">interface</span> <span class="hljs-title class_">ScoreState</span> \{ <span class="hljs-attr">points</span>: <span class="hljs-built_in">number</span> \} <span class="hljs-comment">// exposes this.points</span>

<span class="hljs-keyword">class</span> <span class="hljs-title class_">Scoreboard</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_ inherited__">Component</span>&lt;\{\}, <span class="hljs-title class_">ScoreState</span>&gt; \{
    <span class="hljs-title function_">onCreate</span>(<span class="hljs-params"></span>) \{
        <span class="hljs-variable language_">this</span>.<span class="hljs-property">state</span> = <span class="hljs-keyword">new</span> <span class="hljs-title class_">ScoreState</span>(\{ <span class="hljs-attr">points</span>: <span class="hljs-number">0</span> \});
        <span class="hljs-variable language_">this</span>.<span class="hljs-property">state</span>.<span class="hljs-property">points</span>++; <span class="hljs-comment">// ✅ typed, no cast</span>
    \}
\}</code></pre>
<h3 id="models">Models</h3>
<p>Type the attributes with <code>Model&lt;YourAttrs&gt;</code>. Use <strong>declaration merging</strong> to surface the auto-generated getters/setters as instance properties:</p>
<pre><code class="ts language-ts"><span class="hljs-keyword">import</span> \{ <span class="hljs-title class_">Model</span> \} <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;rasti&#x27;</span>;

<span class="hljs-keyword">interface</span> <span class="hljs-title class_">TodoAttrs</span> \{ <span class="hljs-attr">title</span>: <span class="hljs-built_in">string</span>; <span class="hljs-attr">completed</span>: <span class="hljs-built_in">boolean</span>; \}

<span class="hljs-keyword">class</span> <span class="hljs-title class_">Todo</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_ inherited__">Model</span>&lt;<span class="hljs-title class_">TodoAttrs</span>&gt; \{
    <span class="hljs-title function_">preinitialize</span>(<span class="hljs-params"></span>) \{
        <span class="hljs-variable language_">this</span>.<span class="hljs-property">defaults</span> = \{ <span class="hljs-attr">title</span>: <span class="hljs-string">&#x27;&#x27;</span>, <span class="hljs-attr">completed</span>: <span class="hljs-literal">false</span> \};
    \}
    <span class="hljs-title function_">toggle</span>(<span class="hljs-params"></span>) \{ <span class="hljs-variable language_">this</span>.<span class="hljs-property">completed</span> = !<span class="hljs-variable language_">this</span>.<span class="hljs-property">completed</span>; \}
\}
<span class="hljs-keyword">interface</span> <span class="hljs-title class_">Todo</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_">TodoAttrs</span> \{\} <span class="hljs-comment">// Exposes this.title, this.completed</span>

<span class="hljs-keyword">const</span> t = <span class="hljs-keyword">new</span> <span class="hljs-title class_">Todo</span>(\{ <span class="hljs-attr">title</span>: <span class="hljs-string">&#x27;x&#x27;</span> \});
t.<span class="hljs-property">title</span>.<span class="hljs-title function_">toUpperCase</span>(); <span class="hljs-comment">// ✅</span>
t.<span class="hljs-title function_">on</span>(<span class="hljs-string">&#x27;change:completed&#x27;</span>, <span class="hljs-function">(<span class="hljs-params">m, value</span>) =&gt;</span> value &amp;&amp; <span class="hljs-comment">/* boolean */</span> <span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(<span class="hljs-string">&#x27;done&#x27;</span>));</code></pre>
<h3 id="helper-types">Helper types</h3>
<pre><code class="ts language-ts"><span class="hljs-keyword">import</span> \{
    <span class="hljs-title class_">EventHandler</span>,
    <span class="hljs-title class_">RenderExpression</span>,
    <span class="hljs-title class_">ModelAttrs</span>,
    <span class="hljs-title class_">ComponentProps</span>,
    <span class="hljs-title class_">ComponentState</span>,
    <span class="hljs-title class_">ComponentModel</span>,
\} <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;rasti&#x27;</span>;

<span class="hljs-comment">// \`Counter\` (defined above with Component.create) is a *value*. To use the name in</span>
<span class="hljs-comment">// type position, alias it once — now \`Counter\` is both a value and a type:</span>
<span class="hljs-keyword">type</span> <span class="hljs-title class_">Counter</span> = <span class="hljs-title class_">InstanceType</span>&lt;<span class="hljs-keyword">typeof</span> <span class="hljs-title class_">Counter</span>&gt;;

<span class="hljs-comment">// Typed event handler with \`this\` bound to the component</span>
<span class="hljs-keyword">const</span> <span class="hljs-attr">onClick</span>: <span class="hljs-title class_">EventHandler</span>&lt;<span class="hljs-title class_">Counter</span>, <span class="hljs-title class_">MouseEvent</span>&gt; = <span class="hljs-keyword">function</span>(<span class="hljs-params">ev</span>) \{
    <span class="hljs-variable language_">this</span>.<span class="hljs-property">props</span>.<span class="hljs-property">initial</span>;
\};

<span class="hljs-comment">// Typed render expression (\`(component) =&gt; any\`)</span>
<span class="hljs-keyword">const</span> <span class="hljs-attr">renderLabel</span>: <span class="hljs-title class_">RenderExpression</span>&lt;<span class="hljs-title class_">Counter</span>&gt; = <span class="hljs-function">(<span class="hljs-params">\{ props \}</span>) =&gt;</span> props.<span class="hljs-property">initial</span>;

<span class="hljs-comment">// Extract types from existing classes</span>
<span class="hljs-keyword">type</span> A = <span class="hljs-title class_">ModelAttrs</span>&lt;<span class="hljs-title class_">Todo</span>&gt;;        <span class="hljs-comment">// Todo extends Model → already a type, no alias</span>
<span class="hljs-keyword">type</span> P = <span class="hljs-title class_">ComponentProps</span>&lt;<span class="hljs-title class_">Counter</span>&gt;; <span class="hljs-comment">// pass the instance; \`ComponentProps&lt;typeof Counter&gt;\` is \`never\`</span>
<span class="hljs-keyword">type</span> S = <span class="hljs-title class_">ComponentState</span>&lt;<span class="hljs-title class_">Counter</span>&gt;;</code></pre>
<blockquote>
  <p>Components made with <code>Component.create</code> are <strong>values</strong>, not types. To use one as a type — as with <code>Counter</code> above — add <code>type X = InstanceType&lt;typeof X&gt;</code> next to the definition, or write <code>InstanceType&lt;typeof X&gt;</code> inline. A <code>Model</code> subclass needs no alias, since <code>class</code> already declares both a value and a type.</p>
</blockquote>
<h3 id="typing-template-interpolations">Typing template interpolations</h3>
<p>Functions inside a template are <code>any</code> — rasti can't infer them from the surrounding string. Which type to use depends on how rasti treats the function (quoted attribute or content → run on render; unquoted attribute → passed as-is):</p>
<blockquote>
  <p>Under <code>strict</code> / <code>noImplicitAny</code>, every interpolation callback <strong>must</strong> be annotated — an untyped parameter is an error (TS7031/TS7006), not a silent <code>any</code>. In non-strict mode typing is opt-in: annotate where you want safety and leave trivial ones as <code>any</code>.</p>
</blockquote>
<table>
<thead>
<tr>
...
<tbody>
<tr>
<td>Content <code>\$\{fn\}</code> or quoted attr <code>attr="\$\{fn\}"</code></td>
<td>Run on render; <code>this</code> and the argument are the component</td>
<td><code>RenderExpression&lt;C&gt;</code></td>
</tr>
<tr>
<td>Unquoted <code>onX=\$\{fn\}</code></td>
<td>DOM handler, called <code>(event, component, matched)</code></td>
<td><code>EventHandler&lt;C, E&gt;</code></td>
</tr>
<tr>
<td>Function passed to a child (<code>handler=\$\{fn\}</code>)</td>
<td>Becomes the child's prop; typed by the child, not this component</td>
<td>the child's prop signature</td>
</tr>
</tbody>
</table>
<p>Three ways to apply them:</p>
<pre><code class="ts language-ts"><span class="hljs-comment">// \`Home\` is a value (made with Component.create), so alias it to use the name as a type:</span>
<span class="hljs-keyword">const</span> <span class="hljs-title class_">Home</span> = <span class="hljs-title class_">Component</span>.<span class="hljs-property">create</span>&lt;\{\}, \{ <span class="hljs-attr">location</span>: <span class="hljs-built_in">string</span> \}&gt;<span class="hljs-string">\`&lt;div&gt;&lt;/div&gt;\`</span>.<span class="hljs-title function_">extend</span>(\{
    <span class="hljs-title function_">close</span>(<span class="hljs-params"></span>) \{ <span class="hljs-comment">/* ... */</span> \},
\});
<span class="hljs-keyword">type</span> <span class="hljs-title class_">Home</span> = <span class="hljs-title class_">InstanceType</span>&lt;<span class="hljs-keyword">typeof</span> <span class="hljs-title class_">Home</span>&gt;;

<span class="hljs-comment">// 1. Named const — cleanest for non-trivial handlers</span>
<span class="hljs-keyword">const</span> <span class="hljs-attr">onClick</span>: <span class="hljs-title class_">EventHandler</span>&lt;<span class="hljs-title class_">Home</span>, <span class="hljs-title class_">MouseEvent</span>&gt; = <span class="hljs-keyword">function</span>(<span class="hljs-params">ev, self</span>) \{
    ev.<span class="hljs-title function_">preventDefault</span>();
    self.<span class="hljs-title function_">close</span>();
\};

<span class="hljs-comment">// 2. Inline with \`satisfies\` — checks + types the params without widening</span>
\$\{(<span class="hljs-function">(<span class="hljs-params">\{ state \}</span>) =&gt;</span> state?.<span class="hljs-property">location</span>) <span class="hljs-keyword">satisfies</span> <span class="hljs-title class_">RenderExpression</span>&lt;<span class="hljs-title class_">Home</span>&gt;\}

<span class="hljs-comment">// 3. Bare annotation — lightest, just types the argument</span>
\$\{<span class="hljs-function">(<span class="hljs-params">\{ state \}: <span class="hljs-title class_">Home</span></span>) =&gt;</span> state?.<span class="hljs-property">location</span>\}</code></pre>
<p>For a function passed to a child, neither helper fits — its type comes from the child's prop. Type it against that prop's declared type (rasti can't connect the attribute to the child, since both live inside the template string):</p>
<pre><code class="ts language-ts"><span class="hljs-comment">// where the child was created with Component.create&lt;ToggleAllProps&gt;\`...\`</span>
handleChange=\$\{(<span class="hljs-function">(<span class="hljs-params">checked</span>) =&gt;</span> model.<span class="hljs-title function_">toggleAll</span>(checked)) <span class="hljs-keyword">satisfies</span> <span class="hljs-title class_">ToggleAllProps</span>[<span class="hljs-string">&#x27;handleChange&#x27;</span>]\}</code></pre>
<h3 id="known-limitations">Known limitations</h3>
<ul>
<li><strong>Template interpolation callbacks are <code>any</code></strong>. Functions in <code>Component.create\</code>…\`\` templates can't be inferred from the surrounding string — type them opt-in (see <a href="#typing-template-interpolations">Typing template interpolations</a>).</li>
<li><strong><code>Model&lt;A&gt;</code> instance keys require declaration merging</strong>. TypeScript can't add <code>A</code>'s keys to a <code>class extends Model&lt;A&gt;</code> automatically — see the <code>interface Todo extends TodoAttrs \{\}</code> pattern above.</li>
<li><strong><code>this.\$()</code> can return <code>null</code></strong>. It mirrors <code>querySelector</code>, so handle the empty case (<code>?.</code>) and pass a type argument to narrow the element: <code>this.\$&lt;HTMLInputElement&gt;('input.edit')?.focus()</code>. <code>this.\$\$()</code> returns a <code>NodeListOf&lt;HTMLElement&gt;</code> (also narrowable).</li>
<li><strong><code>this.model</code> / <code>this.state</code> are optional</strong>. Both are <code>undefined</code> unless provided, so guard (<code>this.model?.foo</code>) or assert (<code>this.model!</code>) when you know one was passed. Both accept a Rasti <code>Model</code> or a model from another library (e.g. Backbone); Components subscribe to <code>change</code> events automatically when the object exposes <code>on</code>/<code>off</code>.</li>
<li><strong><code>state</code> / <code>model</code> are raw generics, <code>props</code> is not</strong>. <code>this.props</code> is <em>always</em> a <code>Model</code> built by rasti, so it's typed <code>Model&lt;P&gt; &amp; P</code> (direct access to <code>P</code>'s keys). But <code>state</code> and <code>model</code> can be anything you provide — a Rasti <code>Model</code>, a Backbone model, a store, or a plain object — so they stay the raw generic. To read a typed <code>Model</code> state/model directly, define it as a named subclass with declaration merging and pass it as the <code>S</code>/<code>M</code> generic (see the <code>Scoreboard</code> example above) — no casts needed.</li>
<li><strong>Weak-type error on narrow props</strong>. If a component's <code>P</code> has no required keys and you pass only options not declared in it, TypeScript reports <em>"has no properties in common"</em> (weak-type check). Fix: declare those options in <code>P</code> — non-reserved options become props at runtime.</li>
<li><strong>Instance fields set in <code>.extend</code> hooks need predeclaration</strong>. <code>.extend</code> infers the instance type from the object's members only, so a field first assigned in <code>onCreate</code> (<code>this.router = ...</code>) isn't known. Predeclare it in the object: <code>router: null as unknown as Router</code>. For components with many instance fields, <code>class MyComponent extends Component&lt;P, S&gt;</code> is usually cleaner than <code>.extend</code>.</li>
</ul>
<h2 id="working-with-llms">Working with LLMs</h2>
<p>For those working with LLMs, there is an <a target="_blank" href="https://github.com/8tentaculos/rasti/blob/master/docs/AGENTS.md">AI Agents reference guide</a> that provides API patterns, lifecycle methods, and best practices, optimized for LLM context. You can share this guide with AI assistants to help them understand <strong>Rasti</strong>'s architecture and component APIs.</p>
<h2 id="changelog">Changelog</h2>
<p>Release history and migration notes for major versions are in <a target="_blank" href="https://github.com/8tentaculos/rasti/blob/master/CHANGELOG.md">CHANGELOG.md</a>.</p></section>
        `;var Xt=y.create`
    <svg class="${({props:e})=>e.className||""}" width="${({props:e})=>e.width||"24"}" height="${({props:e})=>e.height||"24"}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon">
        <path stroke-linecap="round" stroke-linejoin="round" d="m18.75 4.5-7.5 7.5 7.5 7.5m-6-15L5.25 12l7.5 7.5"/>
    </svg>
`;const{classes:me}=A({header:{display:"flex",alignItems:"center",gap:"var(--rui-spacing-xs)",marginBottom:"var(--rui-spacing-sm)",flexShrink:0,...O(e=>({[`&:where([data-variant="solid"][data-color="${e}"])`]:{"& button":{opacity:1,color:`var(--rui-palette-${e}-contrastMain) !important`}}}))},collapseButton:{width:"32px",height:"32px",padding:0,opacity:.4,transition:"transform 0.2s ease, opacity 0.2s ease"},collapseButtonRotated:{transform:"rotate(180deg)"}}),Gt=y.create`
    <div
        class="${()=>me.header}"
        data-slot="header"
        data-variant="${({props:e})=>e.variant||"outlined"}"
...
`,X=y.create`
    <aside
        class="${({props:e})=>I([Ve.root,e.className])}"
        data-variant="${({props:e})=>e.variant||"outlined"}"
        data-color="${({props:e})=>e.color||"neutral"}"
        data-collapsed="${({props:e})=>e.collapsed||!1}"
        ...
        <div class="${ae.root}">
            ...
            >
                <${M}
                    ...
                    variant="plain"
                    color="neutral"
                    renderLeftIcon=${()=>Qt.mount()}
                    onClick="${({state:a})=>()=>{a.dialogOpen=!0}}"
                />
            </${H}>
            ${({state:a,partial:o})=>a.dialogOpen?o`<${W}
                    className="${ae.apiIndexDialog}"
                    handleClose="${({state:l})=>()=>{l.dialogOpen=!1}}"
                    shadow="lg"
                >
                    <${W.Header}
                        title="API Index"
                    ...
                    />
                    <${W.Content}>
                        <${t} events="${({state:l})=>({"click a":()=>{l.dialogOpen=!1}})}" />
                    </${W.Content}>
                </${W}>`:null}
...
<li><em>static</em><ul>
<li><a href="#module_model_attributeprefix">.attributePrefix</a> : <code>string</code></li></ul></li></ul></li>
...
<li><em>static</em><ul>
<li><a href="#module_view_uid">.uid</a> : <code>number</code></li>
<li><a href="#module_view_sanitize">.sanitize(value)</a></li>
<li><a href="#module_view_resetuid">.resetUid()</a></li></ul></li></ul></li>
</ul>
</nav>
        `,Ie=e=>({fontFamily:`var(--rui-typography-${e}-fontFamily)`,fontWeight:`var(--rui-typography-${e}-fontWeight)`,fontSize:`var(--rui-typography-${e}-fontSize)`,lineHeight:`var(--rui-typography-${e}-lineHeight)`}),{classes:ns}=A({root:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-xs)",fontSize:"var(--rui-fontSize-md)",lineHeight:"var(--rui-lineHeight-md)",padding:"var(--rui-spacing-lg)",margin:"0 auto",maxWidth:"var(--rui-app-maxWidth)",color:"var(--rui-palette-neutral-foregroundLevel2)","@global":{h1:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-xl)",fontSize:"var(--rui-fontSize-xxxl)",lineHeight:"var(--rui-lineHeight-xs)",color:"var(--rui-palette-neutral-foregroundLevel1)",margin:"var(--rui-spacing-lg) 0"},h2:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-xl)",fontSize:"var(--rui-fontSize-xxl)",lineHeight:"var(--rui-lineHeight-xs)",color:"var(--rui-palette-neutral-foregroundLevel1)",margin:"var(--rui-spacing-lg) 0",padding:"var(--rui-spacing-sm) 0",borderBottom:"1px solid rgba(var(--rui-palette-neutral-rgb-foregroundLevel1) / 0.2)"},h3:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-lg)",fontSize:"var(--rui-fontSize-xl)",lineHeight:"var(--rui-lineHeight-xs)",color:"var(--rui-palette-neutral-foregroundLevel2)",margin:"var(--rui-spacing-lg) 0",overflowY:"hidden",overflowX:"auto"},h4:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-lg)",fontSize:"var(--rui-fontSize-lg)",lineHeight:"var(--rui-lineHeight-md)",color:"var(--rui-palette-neutral-foregroundLevel3)",margin:"var(--rui-spacing-lg) 0"},h5:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-md)",fontSize:"var(--rui-fontSize-xl)",lineHeight:"var(--rui-lineHeight-md)",color:"var(--rui-palette-neutral-foregroundLevel3)",margin:"var(--rui-spacing-lg) 0"},p:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-xs)",fontSize:"var(--rui-fontSize-md)",lineHeight:"var(--rui-lineHeight-md)",color:"var(--rui-palette-neutral-foregroundLevel2)",margin:"var(--rui-spacing-lg) 0"},li:{fontFamily:"var(--rui-fontFamily-body)",fontWeight:"var(--rui-fontWeight-xs)",fontSize:"var(--rui-fontSize-md)",lineHeight:"var(--rui-lineHeight-md)",color:"var(--rui-palette-neutral-foregroundLevel2)",margin:"var(--rui-spacing-lg) 0"},"li::marker":{color:"var(--rui-palette-neutral-foregroundLevel3)"},code:{fontFamily:"var(--rui-fontFamily-code)",fontSize:"0.875em",lineHeight:"var(--rui-lineHeight-sm)",color:"var(--rui-palette-neutral-foregroundLevel1)",background:"rgb(var(--rui-palette-neutral-rgb-foregroundLevel3) / 0.12)",border:"1px solid rgb(var(--rui-palette-neutral-rgb-foregroundLevel3) / 0.16)",borderRadius:"var(--rui-borderRadius-xs)",padding:"0.1em 0.35em",wordBreak:"break-word"},"a > code":{color:"inherit"},"pre > code":{fontSize:"var(--rui-fontSize-sm)",lineHeight:"var(--rui-lineHeight-md)",borderRadius:"var(--rui-borderRadius-sm)",border:"none",boxShadow:"var(--rui-shadow-xs)",display:"block",background:"#282c34",color:"#abb2bf",overflowX:"auto",padding:"1em",wordBreak:"normal"},a:{color:"var(--rui-palette-primary-foregroundMain)",textDecoration:"none","&:visited, &:active":{color:"var(--rui-palette-primary-foregroundMain)"},"&:hover":{textDecoration:"underline",color:"var(--rui-palette-primary-foregroundDark)"}},table:{color:"var(--rui-palette-neutral-foregroundLevel1)",display:"block",overflowX:"auto",borderCollapse:"collapse","& th":{borderColor:"rgb(var(--rui-palette-neutral-rgb-foregroundLevel3) / 0.2)",borderStyle:"none",padding:"var(--rui-spacing-sm) var(--rui-spacing-md)",...Ie("titleMd"),"& div":{display:"flex",alignItems:"center",justifyContent:"space-evenly"},"& svg:first-child":{padding:"0 var(--rui-spacing-xs) 0 0"},"& svg:last-child":{padding:"0 0 0 var(--rui-spacing-xs)"},"& svg:only-child":{padding:"0"}},"& td":{borderColor:"rgb(var(--rui-palette-neutral-rgb-foregroundLevel3) / 0.2)",borderStyle:"none",padding:"var(--rui-spacing-sm) var(--rui-spacing-md)",...Ie("bodyMd")},"& thead th, & thead td":{borderBottomStyle:"solid",borderBottomWidth:"2px"},"& tfoot th, & tfoot td":{borderTopStyle:"solid",borderTopWidth:"2px"},"& tr:not(:last-child) td":{borderBottomStyle:"solid",borderBottomWidth:"1px"},"& td:not(:last-child), & th:not(:last-child)":{borderRightStyle:"solid",borderRightWidth:"1px"}}},margin:0,width:"100%",boxSizing:"border-box"}}),os=e=>[e.className||null,ns.root].join(" "),ls=y.create`
            <section class="${({props:e})=>os(e)}"><p><a name="module_component" id="module_component" class="anchor"></a></p>
<h2 id="component-⇐-c30c">Component ⇐ <code>View</code></h2>
...
<h3 id="componentsubscribemodel-type-listener-⇒-c45c">component.subscribe(model, [type], [listener]) ⇒ <code>Component</code></h3>
...
<h3 id="componentpartialstrings-expressions-⇒-c53cnew_partial_new">component.partial(strings, …expressions) ⇒ <a href="#new_partial_new"><code>Partial</code></a></h3>
...
<h3 id="componenttostring-⇒-c58c">component.toString() ⇒ <code>string</code></h3>
...
<h3 id="componentrender-⇒-c61c">component.render() ⇒ <code>Component</code></h3>
...
<h3 id="componentonchangemodel-changed">component.onChange(model, changed)</h3>
...
<h3 id="componentmarkassafehtmlvalue-⇒-c77cnew_safehtml_new">Component.markAsSafeHTML(value) ⇒ <a href="#new_safehtml_new"><code>SafeHTML</code></a></h3>
...
<h3 id="componentmountoptions-el-hydrate-⇒-c84c">Component.mount([options], [el], [hydrate]) ⇒ <code>Component</code></h3>
...
<h3 id="componentcreatestrings-expressions-⇒-c92c">Component.create(strings, …expressions) ⇒ <code>Component</code></h3>
...
<h2 id="inverse-of-control-pattern">Inverse of Control Pattern</h2>
...
<h3 id="emitterontype-listener-⇒-c102c">emitter.on(type, listener) ⇒ <code>function</code></h3>
...
<h3 id="emitteroncetype-listener-⇒-c107c">emitter.once(type, listener) ⇒ <code>function</code></h3>
...
<h3 id="emitterofftype-listener">emitter.off([type], [listener])</h3>
...
<h3 id="emitteremittype-args">emitter.emit(type, […args])</h3>
...
<h3 id="emitterlistentoemitter-type-listener-⇒-c118c">emitter.listenTo(emitter, type, listener) ⇒ <code>function</code></h3>
...
<h3 id="emitterlistentoonceemitter-type-listener-⇒-c124c">emitter.listenToOnce(emitter, type, listener) ⇒ <code>function</code></h3>
...
<h3 id="emitterstoplisteningemitter-type-listener">emitter.stopListening([emitter], [type], [listener])</h3>
...
<h2 id="model-⇐-c134c">Model ⇐ <code>Emitter</code></h2>
<ul>
...
<h2 id="construction-flow">Construction Flow</h2>
<ol>
...
<h3 id="modelpreinitializeattributes-args">model.preinitialize([attributes], […args])</h3>
...
<h3 id="modelgetkey-⇒-c154c">model.get(key) ⇒ <code>any</code></h3>
...
<h3 id="modelsetkey-value-args-⇒-c158c">model.set(key, [value], […args]) ⇒ <code>Model</code></h3>
...
<h3 id="modelparsedata-args-⇒-c167c">model.parse([data], […args]) ⇒ <code>object</code></h3>
...
<h3 id="modeltojson-⇒-c173c">model.toJSON() ⇒ <code>object</code></h3>
...
<p><a name="module_model_attributeprefix" id="module_model_attributeprefix" class="anchor"></a></p>
<h3 id="modelattributeprefix--c176c">Model.attributePrefix : <code>string</code></h3>
<p>Static property that defines a prefix for generated getters/setters.
When set, all attribute properties will be prefixed (e.g., 'attr_name' instead of 'name').
Useful for avoiding naming conflicts or creating a consistent property naming convention.</p>
<p><strong>Kind</strong>: static property of <a href="#module_model"><code>Model</code></a><br />
<strong>Default</strong>: <code>"''"</code><br />
<strong>Example</strong>  </p>
<pre><code class="js language-js"><span class="hljs-comment">// Set prefix for all models of this class</span>
<span class="hljs-keyword">class</span> <span class="hljs-title class_">ApiModel</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_ inherited__">Model</span> \{
    <span class="hljs-keyword">static</span> attributePrefix = <span class="hljs-string">&#x27;attr_&#x27;</span>;
\}

<span class="hljs-keyword">const</span> user = <span class="hljs-keyword">new</span> <span class="hljs-title class_">ApiModel</span>(\{ name : <span class="hljs-string">&#x27;Alice&#x27;</span>, email : <span class="hljs-string">&#x27;alice@example.com&#x27;</span> \});
<span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(user.<span class="hljs-property">attr_name</span>);  <span class="hljs-comment">// &#x27;Alice&#x27;</span>
<span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(user.<span class="hljs-property">attr_email</span>); <span class="hljs-comment">// &#x27;alice@example.com&#x27;</span>

<span class="hljs-comment">// Still access via get/set methods without prefix</span>
<span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(user.<span class="hljs-title function_">get</span>(<span class="hljs-string">&#x27;name&#x27;</span>)); <span class="hljs-comment">// &#x27;Alice&#x27;</span>
user.<span class="hljs-title function_">set</span>(<span class="hljs-string">&#x27;name&#x27;</span>, <span class="hljs-string">&#x27;Bob&#x27;</span>);
<span class="hljs-variable language_">console</span>.<span class="hljs-title function_">log</span>(user.<span class="hljs-property">attr_name</span>); <span class="hljs-comment">// &#x27;Bob&#x27;</span></code></pre>
<p><a name="module_view" id="module_view" class="anchor"></a></p>
<h2 id="view-⇐-c179c">View ⇐ <code>Emitter</code></h2>
<ul>
...
<h3 id="viewselector-⇒-c207c">view.\$(selector) ⇒ <code>node</code></h3>
...
<p><a name="module_view__\$\$" id="module_view__\$\$" class="anchor"></a></p>
<h3 id="viewselector-⇒-c211c">view.\$\$(selector) ⇒ <code>Array.&lt;node&gt;</code></h3>
...
<h3 id="viewdestroyoptions-⇒-c215c">view.destroy(options) ⇒ <code>View</code></h3>
<p>Destroy the view.
...
<h3 id="viewaddchildchild-⇒-c221c">view.addChild(child) ⇒ <code>View</code></h3>
...
<h3 id="viewcreateelementtag-attributes-⇒-c227c">view.createElement(tag, attributes) ⇒ <code>node</code></h3>
<p>Create an element.
...
<h3 id="viewremoveelement-⇒-c233c">view.removeElement() ⇒ <code>View</code></h3>
...
<h3 id="viewdelegateeventsevents-⇒-c236c">view.delegateEvents([events]) ⇒ <code>View</code></h3>
...
<h3 id="viewundelegateevents-⇒-c240c">view.undelegateEvents() ⇒ <code>View</code></h3>
...
<h3 id="viewrender-⇒-c243c">view.render() ⇒ <code>View</code></h3>
...
<pre><code class="js language-js"><span class="hljs-keyword">class</span> <span class="hljs-title class_">UserView</span> <span class="hljs-keyword">extends</span> <span class="hljs-title class_ inherited__">View</span> \{
    ...
    \}
\}</code></pre>
<p><a name="module_view_uid" id="module_view_uid" class="anchor"></a></p>
<h3 id="viewuid--c246c">View.uid : <code>number</code></h3>
<p>Counter for generating unique IDs for view instances.<br />
This is primarily used to assign unique identifiers to each view instance (<code>this.uid</code>), which can be helpful for tasks like 
generating element IDs.<br />
<a href="#module_component">Component</a>s use <code>this.uid</code> to generate data attributes for their elements, to be looked up on hydration.<br />
For server-side rendering, this counter should be reset to <code>0</code> on every request to ensure that the generated 
unique IDs match those on the client, enabling seamless hydration of components.</p>
<p><strong>Kind</strong>: static property of <a href="#module_view"><code>View</code></a><br />
<strong>Default</strong>: <code>0</code><br />
<a name="module_view_sanitize" id="module_view_sanitize" class="anchor"></a></p>
<h3 id="viewsanitizevalue-⇒-c249c">View.sanitize(value) ⇒ <code>string</code></h3>
...
<p><strong>Kind</strong>: static method of <a href="#module_view"><code>View</code></a>  </p></section>
        `,rs=es({ApiIndex:as,Api:ls}),{AppBar:is,AppBarMenuContent:cs}=Ht({logoAlt:"Rasti",playgroundUrl:"https://codepen.io/8tentaculos/pen/gOQxaOE?editors=0010",githubUrl:"https://github.com/8tentaculos/rasti",npmUrl:"https://www.npmjs.com/package/rasti"}),ds=Ct({title:"Rasti",AppBar:is,AppBarMenuContent:cs,Cover:Nt,Description:Ft,Features:Vt,Readme:Jt,AboutSite:It({projectName:"Rasti",projectGithubUrl:"https://github.com/8tentaculos/rasti"}),Api:rs,Footer:zt({licenseUrl:"https://github.com/8tentaculos/rasti/blob/master/LICENSE",startYear:2018})});ds.mount(window.__APP_OPTIONS__,document.getElementById("root"),!0);
