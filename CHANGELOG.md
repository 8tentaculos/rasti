...
- **Development warnings for lists**: rendering a list warns once per call site when an item is not a keyed component — so it is regenerated on every update rather than reused — or when two items share a key. Both point at the interpolation in the template source, and neither reaches production builds.
