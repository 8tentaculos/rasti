...
- **Development warnings for lists**: rendering a list warns once per call site when a component in it has no key — so it is built again on every update rather than reused — when two items share a key, or when a key is written under the markup of an item, where it identifies nothing among the list's siblings. Each points at the interpolation in the template source, and none reaches production builds.
- **Mixed attribute values**: a quoted attribute value may compose literal text and any number of interpolations — `class="row ${({ state }) => state.active ? 'active' : ''}"` — joined into a single string. Each interpolated part is evaluated like any quoted value and coerced with the content-interpolation rule: `null`, `undefined`, `true` and `false` render as the empty string, anything else as its string form. A value that is exactly one interpolation keeps passing its raw resolved value, so booleans, objects and handler thunks work as before.
- **Development warnings for unsupported attribute forms**: an interpolation in an attribute name (`data-${x}="1"`) or in an unquoted value mixed with literal text (`attr=x${fn}`) used to fail silently; it now warns once per call site in development, pointing at the offending expression in the template source. Neither form reaches production builds.
- **`Hydration mismatch` error in development**: hydrating server-rendered markup that does not hold what the client renders now throws where the mismatch is, naming the element it expected and the one it found, instead of leaving a slot attached to nothing and failing later somewhere unrelated. Server and client must render the same template from the same version of Rasti. Development only; it adds nothing to production builds.
- **The default `template()` builds a view-style root element**: a component without a template renders `this.tag` (defaulting to `div`) carrying the component's `attributes`, with the content the parent slotted into it (`props.renderChildren`) inside — so `Component.mount({ tag : 'section', attributes : { class : 'panel' } }, el)` renders `<section class="panel"></section>` where it used to render an empty `<div>`. A void tag (`img`, `input`, …) renders self-closed and throws in development if content is passed to it. As in a view, the tag is read on the first render, when the element is created; later renders patch that element and leave its tag alone.
...
- **Mounting is linear in the size of what is rendered**: attaching a render to the DOM used to search the document once per element and per marker it wrote, so the cost grew with the square of the tree — nested components and long lists paid for it on every mount. A render now takes a single snapshot of the nodes it produced and hands it down through the whole subtree; each slot attaches to its own nodes in one walk, in document order, and nothing is searched for. Regenerating an interpolation goes through the same path, so replacing a region got cheaper with it.
- **`template()` runs on every render**, where previously it ran once. This is what makes value-style interpolations reactive.
- **The default export is the base `Component` class**, with a default `template()` that builds the root element the way a view does. `import Component from 'rasti'` still works the same for `create` / `extend` / `mount`.
- **Serialized markup changed**: partials now emit their own element id (`data-rst-el`) and start/end markers, so the exact HTML a component serializes to differs from previous versions. A partial that is nothing but a component tag is the exception: ``partial`<${Comp} />` `` writes no markers of its own and serializes exactly like mounting the component, which is what keeps a list of them free of extra markup. Server-side rendering and hydration are unaffected as long as server and client run the same version; snapshot tests comparing exact HTML must be updated.
- **BREAKING:** **Attribute values escape by origin**: a value is HTML source only when it is a pure literal written in the template, and serializes as written, character references included. Everything else — dynamic, mixed, spread, `attributes` — is plain text and escapes `&` on serialization, so it reaches the DOM as written and matches what `setAttribute` produces on update. The serialized string of a plain-text value holding `&` changes (`?a=1&b=2` serializes as `?a=1&amp;b=2`); the DOM value is unaffected, but server-side output and exact-HTML comparisons are.
- **Attribute serialization escapes U+00A0**: a raw U+00A0 in an attribute value serializes as `&nbsp;`, completing the HTML fragment serialization algorithm the serializer follows (`&`, U+00A0 and the double quote). The DOM value is unaffected.
- **Lists of components are placed by walking them**: an interpolation whose items all stand for a component — a mounted child or a component tag — keeps its children in document order and places them by that order. A child the list keeps writes no markup at all and is moved only if the order moved it, the ones it mounts arrive together in one fragment, and the ones it drops are removed. Where every row used to be written out and put back in place on every update, only what actually changed is touched.
- **BREAKING:** **A list of one template repeated is updated by position**: when every item of a list is a partial of the same template — ``items.map(item => partial`<li>${item.label}</li>`)`` — the items are now patched where they stand instead of the list being regenerated. The ones both renders hold take the new expressions in place, a longer list renders only what it gained, and a shorter one drops the nodes of what it lost. Such items carry no key and therefore no identity, so the DOM stays with the position and the content moves through it: what a render writes — text, attributes, event listeners — follows the content, while what the browser owns — focus, scroll, selection, an open `<details>`, the value of an uncontrolled input — stays with the node. Where the list only grows, shrinks or changes content this preserves state that used to be lost on every update; where the list reorders, that state stays behind with the position. An item that must keep its identity has to be a keyed component, which the list then holds directly (``items.map(item => partial`<${Row} key="${item.id}" />`)``).
- **BREAKING:** **The recycling model is slot-based**: a partial is reconciled by the identity of its call site, and each interpolation reconciles its own region — an occupant that keeps its identity (the same partial, the same component type) is patched in place, anything else is regenerated, and when an intermediate is regenerated everything below goes with it. Recycling by position within a partial works as it did before; what changed is the scope of a `key`. It used to be matched against every child of the component, on a render that regenerated the whole template, so a keyed child could travel between interpolations and out of a partial that was being regenerated. It now identifies a child only among the siblings of its own interpolation, so a keyed child that moves elsewhere — or that sits inside a partial that is regenerated, as in ``items.map(item => partial`<li><${Row} key="${item.id}" /></li>`)`` — is recreated instead of recycled. Keep the keyed component as the item itself (``items.map(item => partial`<${Row} key="${item.id}" />`)``) for it to be reused and moved into place.
